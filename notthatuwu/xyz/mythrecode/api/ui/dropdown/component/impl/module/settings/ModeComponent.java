...
public final class ModeComponent extends ExpandableComponent implements SettingComponent {
  ...
  public ModeComponent(Component parent, ModeSetting property, int x, int y, int width, int height) {
  ...
  public void renderComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
    ...
    String selectedText = this.property.getValue();
    int dropDownBoxY = y + 10;
    boolean needScissor = (FontLoaders.Sfui20.getStringWidth(selectedText) > width - 4);
    ...
    FontLoaders.Sfui20.drawString(getName(), x + 10.5F, dropDownBoxY + 2.8F, -1);
    FontLoaders.Sfui20.drawString(selectedText, (x + 109 - FontLoaders.Sfui20.getStringWidth(selectedText)), dropDownBoxY + 2.8F, ClickGUI.extra.getColor());
    if (needScissor)
  ...
  public void onMouseClick(int mouseX, int mouseY, int button) {
    ...
    if (button == 1)
      this.animation.animate(isExpanded() ? 1.0D : 0.0D, (20 * (getProperty().getOptions()).length), Easings.NONE); 
    if (isExpanded())
  ...
  private <T extends Enum<T>> void handleRender(int x, int y, int width, int textColor) {
    int enabledColor = (new Color(106, 90, 205)).getRGB();
    ModeSetting property = this.property;
    for (String e : property.getOptions()) {
      ClickGuiMod ClickGUI = (ClickGuiMod)Client.INSTANCE.moduleManager.getModuleByClass(ClickGuiMod.class);
      FontLoaders.Sfui20.drawString(StringUtil.upperSnakeCaseToPascal(e), (x + 1 + calculateMiddle(StringUtil.upperSnakeCaseToPascal(e), FontLoaders.Sfui20, 54.0D, 1.0D)), y, property.getValue().equalsIgnoreCase(e) ? ClickGUI.extra.getColor() : -1);
      y += 12;
    } 
  }
  ...
  public int getHeightus() {
    return (this.property.getOptions()).length * 12;
  }
  ...
  public boolean canExpand() {
    return ((this.property.getOptions()).length > 1);
  }
}
