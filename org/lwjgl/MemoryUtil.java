...
public final class MemoryUtil {
  ...
  static {
    AccessorJNI accessorJNI;
  }
  
  static {
    try {
      Accessor util = loadAccessor("org.lwjgl.MemoryUtilSun$AccessorUnsafe");
    } catch (Exception e0) {
      try {
        Accessor accessor = loadAccessor("org.lwjgl.MemoryUtilSun$AccessorReflectFast");
      } catch (Exception e1) {
        try {
          AccessorReflect accessorReflect = new AccessorReflect();
        } catch (Exception e2) {
          LWJGLUtil.log("Unsupported JVM detected, this will likely result in low performance. Please inform LWJGL developers.");
          accessorJNI = new AccessorJNI(null);
        } 
      } 
    } 
    LWJGLUtil.log("MemoryUtil Accessor: " + accessorJNI.getClass().getSimpleName());
    memUtil = (Accessor)accessorJNI;
  }
