...
public final class Display {
  ...
  private static DisplayImplementation createDisplayImplementation() {
    switch (LWJGLUtil.getPlatform()) {
      case 1:
        return (DisplayImplementation)new LinuxDisplay();
      case 3:
        return new WindowsDisplay();
      case 2:
        return (DisplayImplementation)new MacOSXDisplay();
    } 
  ...
  public static void create(PixelFormatLWJGL pixel_format, Drawable shared_drawable, ContextAttribs attribs) throws LWJGLException {
    synchronized (GlobalLock.lock) {
      ...
      Display.drawable = (DrawableLWJGL)object;
      try {
        object.setPixelFormat(pixel_format);
        try {
          createWindow();
          try {
            object.createContext(attribs, shared_drawable);
            try {
              makeCurrentAndSetSwapInterval();
              initContext();
            } catch (LWJGLException e) {
  ...
  static String getPrivilegedString(String property_name) {
    return AccessController.<String>doPrivileged((PrivilegedAction<String>)new Object(property_name));
  }
