...
public final class ConsoleAppender extends AbstractOutputStreamAppender<OutputStreamManager> {
  ...
  private static OutputStream getOutputStream(boolean follow, boolean direct, Target target) {
    ...
    try {
      OutputStream outputStream = (target == Target.SYSTEM_OUT) ? (direct ? new FileOutputStream(FileDescriptor.out) : (follow ? new PrintStream((OutputStream)new SystemOutStream(), true, enc) : System.out)) : (direct ? new FileOutputStream(FileDescriptor.err) : (follow ? new PrintStream((OutputStream)new SystemErrStream(), true, enc) : System.err));
  ...
  private static class ConsoleAppender {}
