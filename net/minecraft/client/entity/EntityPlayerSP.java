...
public class EntityPlayerSP extends AbstractClientPlayer {
  ...
  public void moveEntity(double x, double y, double z) {
    ...
    if (moveEvent.isCancelled())
      return; 
    TargetStrafe targetStrafeModule = (TargetStrafe)Client.INSTANCE.moduleManager.getModuleByClass(TargetStrafe.class);
    if (targetStrafeModule.isEnabled())
      moveEvent = targetStrafeModule.editMovement(moveEvent.getX(), moveEvent.getY(), moveEvent.getZ()); 
    super.moveEntity(moveEvent.getX(), moveEvent.getY(), moveEvent.getZ());
  }
  
  public boolean isMoving() {
    return ((Minecraft.getMinecraft()).thePlayer.moveStrafing != 0.0F || (Minecraft.getMinecraft()).thePlayer.moveForward != 0.0F);
  }
  
  public void onUpdate() {
    if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
      this.e = new EventUpdate(this.mc.thePlayer.posX, this.mc.thePlayer.posY, this.mc.thePlayer.posZ, this.mc.thePlayer.rotationYaw, this.mc.thePlayer.rotationPitch, this.mc.thePlayer.onGround, EventUpdate.Type.POST);
      this.e.call();
  ...
  public void onUpdateWalkingPlayer() {
    this.e = new EventUpdate(this.mc.thePlayer.posX, this.mc.thePlayer.posY, this.mc.thePlayer.posZ, this.mc.thePlayer.rotationYaw, this.mc.thePlayer.rotationPitch, this.mc.thePlayer.onGround, EventUpdate.Type.PRE);
    this.e.call();
    this.e.setLastPitch(this.lastReportedPitch);
    this.e.setLastYaw(this.lastReportedYaw);
    this.cachedYaw = this.mc.thePlayer.rotationYaw;
    this.cachedPitch = this.mc.thePlayer.rotationPitch;
    if (this.e.isRotatingYaw() && this.mc.gameSettings.thirdPersonView != 0) {
      this.mc.thePlayer.rotationYawHead = this.e.getYaw();
      this.mc.thePlayer.prevRotationYawHead = this.e.getLastYaw();
      this.mc.thePlayer.renderYawOffset = this.e.getYaw();
      this.mc.thePlayer.prevRenderYawOffset = this.e.getLastYaw();
      this.e.setRotatingYaw(false);
    } 
    this.eventPitch = this.e.getPitch();
    this.mc.thePlayer.rotationYaw = this.e.getYaw();
    this.mc.thePlayer.rotationPitch = this.e.getPitch();
    ...
    if (isCurrentViewEntity()) {
      ...
      double d3 = (this.e.getYaw() - this.lastReportedYaw);
      double d4 = (this.e.getPitch() - this.lastReportedPitch);
      boolean flag2 = (d0 * d0 + d1 * d1 + d2 * d2 > 9.0E-4D || this.positionUpdateTicks >= 20);
      boolean flag3 = (d3 != 0.0D || d4 != 0.0D);
      if (this.ridingEntity == null) {
        if (flag2 && flag3) {
          this.sendQueue.addToSendQueue((Packet)new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, (getEntityBoundingBox()).minY, this.posZ, this.e.getYaw(), this.e.getPitch(), this.onGround));
        } else if (flag2) {
          ...
          this.sendQueue.addToSendQueue((Packet)new C03PacketPlayer.C05PacketPlayerLook(this.e.getYaw(), this.e.getPitch(), this.onGround));
        } else {
          this.sendQueue.addToSendQueue((Packet)new C03PacketPlayer(this.onGround));
        } 
      } else {
        this.sendQueue.addToSendQueue((Packet)new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, this.e.getYaw(), this.e.getPitch(), this.onGround));
        flag2 = false;
      } 
      ...
      if (flag3) {
        this.lastReportedYaw = this.e.getYaw();
        this.lastReportedPitch = this.e.getPitch();
      } 
    } 
    this.mc.thePlayer.rotationYaw = this.cachedYaw;
    this.mc.thePlayer.rotationPitch = this.cachedPitch;
    this.e.setType(EventUpdate.Type.POST);
  }
  ...
  public void onLivingUpdate() {
    ...
    if (this.onGround && !flag1 && !flag2)
      if (MovementInput.moveForward >= f && !isSprinting() && flag3 && !isUsingItem() && !isPotionActive(Potion.blindness))
    ...
    if (!isSprinting())
      if (MovementInput.moveForward >= f && flag3 && (!isUsingItem() || ModuleManager.getModuleByName("NoSlow").isEnabled()) && !isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown())
        setSprinting(true);  
    if (isSprinting())
