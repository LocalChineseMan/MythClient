...
public class GuiIngame extends Gui {
  ...
  private BlurShader blurUtil = new BlurShader();
  ...
  public void renderGameOverlay(float partialTicks) {
    ...
    Event2D event2D = new Event2D(scaledresolution);
    event2D.call();
    ...
    if (this.recordPlayingUpFor > 0) {
      ...
      if (k1 > 8) {
        ...
        getFontRenderer().drawString(this.recordPlaying, -getFontRenderer().getStringWidth(this.recordPlaying) / 2, -4, i1 + (k1 << 24 & 0xFF000000));
        GlStateManager.disableBlend();
    ...
    if (scoreobjective1 != null) {
      Scoreboard scoreboard1 = (Scoreboard)Client.INSTANCE.moduleManager.getModuleByClass(Scoreboard.class);
      if (!scoreboard1.hide.getValue().booleanValue() || !scoreboard1.isEnabled())
        renderScoreboard(scoreobjective1, scaledresolution, true); 
    } 
    BetterChat betterChat = (BetterChat)Client.INSTANCE.moduleManager.getModuleByClass(BetterChat.class);
    boolean betterChatBlurBool = betterChat.blur.getValue().booleanValue();
    if (betterChatBlurBool && betterChat.isEnabled()) {
      if (betterChat.dropshadow.getValue().booleanValue()) {
        DropShadowUtil.start();
        doRenderChat(true);
        DropShadowUtil.stop();
        doRenderChat(false);
      } 
      this.blurUtil.startBlur();
      doRenderChat(true);
      this.blurUtil.stopBlur();
      doRenderChat(false);
    } else {
      doRenderChat(true);
      doRenderChat(false);
  ...
  private void doRenderChat(boolean rectangle) {
    ...
    this.persistantChatGUI.drawChat(this.updateCounter, rectangle);
    this.mc.mcProfiler.endSection();
    GlStateManager.popMatrix();
  }
  ...
  private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_, boolean rect) {
    ...
    for (Object score1 : arraylist1) {
      ...
      int chars = 0;
      String str = (this.mc.getCurrentServerData()).serverIP;
      for (char c : str.toCharArray()) {
        if (s1.contains(String.valueOf(c)) && scoreboard1.watermark.getValue().booleanValue())
          chars++; 
      } 
      if (chars == str.length())
        s1 = EnumChatFormatting.YELLOW + "myth"; 
      if (rect)
        Gui.drawRect((j - 2), l, i1, (l + (getFontRenderer()).FONT_HEIGHT), 1342177280); 
