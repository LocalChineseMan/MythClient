...
public class GuiStreamUnavailable extends GuiScreen {
  ...
  protected void actionPerformed(GuiButton button) throws IOException {
    if (button.enabled) {
      if (button.id == 1)
        switch (null.$SwitchMap$net$minecraft$client$gui$stream$GuiStreamUnavailable$Reason[this.field_152326_h.ordinal()]) {
          case 1:
          case 2:
  ...
  public static void func_152321_a(GuiScreen p_152321_0_) {
    ...
    if (!OpenGlHelper.framebufferSupported) {
      ...
      minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.NO_FBO, list));
      ...
      if (((NullStream)istream).func_152937_a().getMessage().contains("Can't load AMD 64-bit .dll on a IA 32-bit platform")) {
        minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.LIBRARY_ARCH_MISMATCH));
      } else {
        minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.LIBRARY_FAILURE));
      } 
    } else if (!istream.func_152928_D() && istream.func_152912_E() == ErrorCode.TTV_EC_OS_TOO_OLD) {
      switch (null.$SwitchMap$net$minecraft$util$Util$EnumOS[Util.getOSType().ordinal()]) {
        case 1:
          minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.UNSUPPORTED_OS_WINDOWS));
          return;
        case 2:
          minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.UNSUPPORTED_OS_MAC));
          return;
      } 
      minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.UNSUPPORTED_OS_OTHER));
      ...
      if (minecraft.getSession().getSessionType() == Session.Type.LEGACY) {
        minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.ACCOUNT_NOT_MIGRATED));
      } else {
        minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.ACCOUNT_NOT_BOUND));
      } 
    } else if (!istream.func_152913_F()) {
      switch (null.$SwitchMap$net$minecraft$client$stream$IStream$AuthFailureReason[istream.func_152918_H().ordinal()]) {
        case 1:
          minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.FAILED_TWITCH_AUTH));
          return;
      } 
      minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.FAILED_TWITCH_AUTH_ERROR));
      ...
      minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.INITIALIZATION_FAILURE, list1));
    } else {
      minecraft.displayGuiScreen((GuiScreen)new GuiStreamUnavailable(p_152321_0_, Reason.UNKNOWN));
    } 
  }
