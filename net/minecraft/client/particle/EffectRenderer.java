...
public class EffectRenderer {
  ...
  private void tickParticle(EntityFX p_178923_1_) {
    try {
      ...
      crashreportcategory.addCrashSectionCallable("Particle", (Callable)new Object(this, p_178923_1_));
      crashreportcategory.addCrashSectionCallable("Particle Type", (Callable)new Object(this, i));
      ...
      for (int k = 0; k < 2; k++) {
        if (!this.fxLayers[j][k].isEmpty()) {
          ...
          for (int l = 0; l < this.fxLayers[j][k].size(); l++) {
            EntityFX entityfx = this.fxLayers[j][k].get(l);
            try {
              entityfx.renderParticle(worldrenderer, entityIn, partialTicks, f, f4, f1, f2, f3);
            } catch (Throwable throwable) {
              CrashReport crashreport = CrashReport.makeCrashReport(throwable, "Rendering Particle");
              CrashReportCategory crashreportcategory = crashreport.makeCategory("Particle being rendered");
              crashreportcategory.addCrashSectionCallable("Particle", (Callable)new Object(this, entityfx));
              crashreportcategory.addCrashSectionCallable("Particle Type", (Callable)new Object(this, j));
              throw new ReportedException(crashreport);
            } 
          } 
          tessellator.draw();
