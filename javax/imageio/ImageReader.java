...
public abstract class ImageReader {
  ...
  protected ImageReader(ImageReaderSpi paramImageReaderSpi) {
    this.originatingProvider = paramImageReaderSpi;
  }
  ...
  public ImageTypeSpecifier getRawImageType(int paramInt) throws IOException {
    return getImageTypes(paramInt).next();
  }
  ...
  private IIOMetadata getMetadata(String paramString, Set paramSet, boolean paramBoolean, int paramInt) throws IOException {
    ...
    if (iIOMetadata != null) {
      if (iIOMetadata.isStandardMetadataFormatSupported() && paramString
  ...
  public Iterator<IIOImage> readAll(Iterator<? extends ImageReadParam> paramIterator) throws IOException {
    ArrayList<IIOImage> arrayList = new ArrayList();
    int i = getMinIndex();
    processSequenceStarted(i);
    while (true) {
      ImageReadParam imageReadParam = null;
      if (paramIterator != null && paramIterator.hasNext()) {
        ImageReadParam imageReadParam1 = (ImageReadParam)paramIterator.next();
        if (imageReadParam1 != null)
          if (imageReadParam1 instanceof ImageReadParam) {
      ...
      ArrayList<BufferedImage> arrayList1 = null;
  ...
  protected void processSequenceStarted(int paramInt) {
    ...
    for (byte b = 0; b < i; b++) {
      IIOReadProgressListener iIOReadProgressListener = this.progressListeners.get(b);
  ...
  protected void processPassStarted(BufferedImage paramBufferedImage, int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int paramInt6, int paramInt7, int[] paramArrayOfint) {
    ...
    for (byte b = 0; b < i; b++) {
      IIOReadUpdateListener iIOReadUpdateListener = this.updateListeners.get(b);
  ...
  protected void processWarningOccurred(String paramString) {
    ...
    for (byte b = 0; b < i; b++) {
      IIOReadWarningListener iIOReadWarningListener = this.warningListeners.get(b);
  ...
  protected void processWarningOccurred(String paramString1, String paramString2) {
    ...
    for (byte b = 0; b < i; b++) {
      IIOReadWarningListener iIOReadWarningListener = this.warningListeners.get(b);
      Locale locale = this.warningLocales.get(b);
      if (locale == null)
        locale = Locale.getDefault(); 
      ClassLoader classLoader = AccessController.<ClassLoader>doPrivileged((PrivilegedAction<ClassLoader>)new Object(this));
      ResourceBundle resourceBundle = null;
      try {
        resourceBundle = ResourceBundle.getBundle(paramString1, locale, classLoader);
      } catch (MissingResourceException missingResourceException) {
        try {
          resourceBundle = ResourceBundle.getBundle(paramString1, locale);
  ...
  protected static BufferedImage getDestination(ImageReadParam paramImageReadParam, Iterator<ImageTypeSpecifier> paramIterator, int paramInt1, int paramInt2) throws IIOException {
    ...
    if (imageTypeSpecifier == null) {
      ...
      if (!(imageTypeSpecifier1 instanceof ImageTypeSpecifier))
        throw new IllegalArgumentException("Non-ImageTypeSpecifier retrieved from imageTypes!"); 
      imageTypeSpecifier = imageTypeSpecifier1;
    } else {
      boolean bool = false;
      while (paramIterator.hasNext()) {
        ImageTypeSpecifier imageTypeSpecifier1 = paramIterator.next();
