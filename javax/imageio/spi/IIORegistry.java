...
public final class IIORegistry extends ServiceRegistry {
  ...
  public void registerApplicationClasspathSpis() {
    ...
    Iterator<Class<?>> iterator = getCategories();
    while (iterator.hasNext()) {
      Class<?> clazz = iterator.next();
      Iterator<IIOServiceProvider> iterator1 = ServiceLoader.load(clazz, classLoader).iterator();
      while (iterator1.hasNext()) {
        try {
          IIOServiceProvider iIOServiceProvider = iterator1.next();
  ...
  private void registerInstalledProviders() {
    PrivilegedAction<?> privilegedAction = new PrivilegedAction() {
        public Object run() {
          Iterator<Class<?>> iterator = IIORegistry.this.getCategories();
          while (iterator.hasNext()) {
            Class<?> clazz = iterator.next();
