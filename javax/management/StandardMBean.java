...
public class StandardMBean implements DynamicMBean, MBeanRegistration {
  ...
  private <T> void construct(T paramT, Class<T> paramClass, boolean paramBoolean1, boolean paramBoolean2) throws NotCompliantMBeanException {
    ...
    if (paramBoolean2) {
      if (paramClass == null)
        paramClass = Util.<Class<T>>cast(Introspector.getMXBeanInterface(paramT
              .getClass())); 
      this.mbean = new MXBeanSupport(paramT, paramClass);
    } else {
      if (paramClass == null)
        paramClass = Util.<Class<T>>cast(Introspector.getStandardMBeanInterface(paramT
              .getClass())); 
  ...
  public void setImplementation(Object paramObject) throws NotCompliantMBeanException {
    ...
    if (isMXBean()) {
      this
        .mbean = new MXBeanSupport((T)paramObject, Util.<Class<T>>cast(getMBeanInterface()));
    } else {
      this
        .mbean = new StandardMBeanSupport((T)paramObject, Util.<Class<T>>cast(getMBeanInterface()));
    } 
  }
  ...
  public Object getAttribute(String paramString) throws AttributeNotFoundException, MBeanException, ReflectionException {
    return this.mbean.getAttribute(paramString);
  }
  ...
  public void setAttribute(Attribute paramAttribute) throws AttributeNotFoundException, InvalidAttributeValueException, MBeanException, ReflectionException {
    this.mbean.setAttribute(paramAttribute);
  }
  
  public AttributeList getAttributes(String[] paramArrayOfString) {
    return this.mbean.getAttributes(paramArrayOfString);
  }
  
  public AttributeList setAttributes(AttributeList paramAttributeList) {
    return this.mbean.setAttributes(paramAttributeList);
  }
  ...
  public Object invoke(String paramString, Object[] paramArrayOfObject, String[] paramArrayOfString) throws MBeanException, ReflectionException {
    return this.mbean.invoke(paramString, paramArrayOfObject, paramArrayOfString);
  }
  ...
  public MBeanInfo getMBeanInfo() {
    try {
      ...
      if (JmxProperties.MISC_LOGGER.isLoggable(Level.FINEST))
        JmxProperties.MISC_LOGGER.logp(Level.FINEST, MBeanServerFactory.class
            .getName(), "getMBeanInfo", "Failed to get cached MBeanInfo", runtimeException); 
    } 
    if (JmxProperties.MISC_LOGGER.isLoggable(Level.FINER))
      JmxProperties.MISC_LOGGER.logp(Level.FINER, MBeanServerFactory.class
          .getName(), "getMBeanInfo", "Building MBeanInfo for " + 
    ...
    boolean bool = immutableInfo((Class)getClass());
    ...
    try {
      ...
      if (JmxProperties.MISC_LOGGER.isLoggable(Level.FINEST))
        JmxProperties.MISC_LOGGER.logp(Level.FINEST, MBeanServerFactory.class
            .getName(), "getMBeanInfo", "Failed to cache MBeanInfo", runtimeException); 
    } 
    return mBeanInfo2;
  }
  ...
  Descriptor getDescriptor(MBeanInfo paramMBeanInfo, boolean paramBoolean) {
    ...
    if (paramMBeanInfo == null || paramMBeanInfo
      .getDescriptor() == null || (paramMBeanInfo
      .getDescriptor().getFieldNames()).length == 0) {
      ...
      immutableDescriptor = new ImmutableDescriptor((Map)hashMap);
    } 
  ...
  protected void cacheMBeanInfo(MBeanInfo paramMBeanInfo) {
    this.cachedMBeanInfo = paramMBeanInfo;
  }
  ...
  private static MBeanParameterInfo customize(MBeanParameterInfo paramMBeanParameterInfo, String paramString1, String paramString2) {
    if (equal(paramString1, paramMBeanParameterInfo.getName()) && 
      equal(paramString2, paramMBeanParameterInfo.getDescription()))
    ...
    if (paramMBeanParameterInfo instanceof OpenMBeanParameterInfo) {
      ...
      return new OpenMBeanParameterInfoSupport(paramString1, paramString2, openMBeanParameterInfo
          
          .getOpenType(), paramMBeanParameterInfo
          .getDescriptor());
    } 
    return new MBeanParameterInfo(paramString1, paramMBeanParameterInfo
        .getType(), paramString2, paramMBeanParameterInfo
  ...
  private static MBeanConstructorInfo customize(MBeanConstructorInfo paramMBeanConstructorInfo, String paramString, MBeanParameterInfo[] paramArrayOfMBeanParameterInfo) {
    if (equal(paramString, paramMBeanConstructorInfo.getDescription()) && 
      identicalArrays(paramArrayOfMBeanParameterInfo, paramMBeanConstructorInfo.getSignature()))
    ...
    if (paramMBeanConstructorInfo instanceof javax.management.openmbean.OpenMBeanConstructorInfo) {
      ...
      return new OpenMBeanConstructorInfoSupport(paramMBeanConstructorInfo.getName(), paramString, arrayOfOpenMBeanParameterInfo, paramMBeanConstructorInfo
          
          .getDescriptor());
    } 
    return new MBeanConstructorInfo(paramMBeanConstructorInfo.getName(), paramString, paramArrayOfMBeanParameterInfo, paramMBeanConstructorInfo
  ...
  private static MBeanOperationInfo customize(MBeanOperationInfo paramMBeanOperationInfo, String paramString, MBeanParameterInfo[] paramArrayOfMBeanParameterInfo, int paramInt) {
    if (equal(paramString, paramMBeanOperationInfo.getDescription()) && 
      identicalArrays(paramArrayOfMBeanParameterInfo, paramMBeanOperationInfo.getSignature()) && paramInt == paramMBeanOperationInfo
      .getImpact())
    ...
    if (paramMBeanOperationInfo instanceof OpenMBeanOperationInfo) {
      ...
      OpenMBeanParameterInfo[] arrayOfOpenMBeanParameterInfo = paramsToOpenParams(paramArrayOfMBeanParameterInfo);
      return new OpenMBeanOperationInfoSupport(paramMBeanOperationInfo.getName(), paramString, arrayOfOpenMBeanParameterInfo, openMBeanOperationInfo
          
          .getReturnOpenType(), paramInt, paramMBeanOperationInfo
          
          .getDescriptor());
    } 
    return new MBeanOperationInfo(paramMBeanOperationInfo.getName(), paramString, paramArrayOfMBeanParameterInfo, paramMBeanOperationInfo
        
        .getReturnType(), paramInt, paramMBeanOperationInfo
  ...
  private static MBeanAttributeInfo customize(MBeanAttributeInfo paramMBeanAttributeInfo, String paramString) {
    ...
    if (paramMBeanAttributeInfo instanceof OpenMBeanAttributeInfo) {
      ...
      return new OpenMBeanAttributeInfoSupport(paramMBeanAttributeInfo.getName(), paramString, openMBeanAttributeInfo
          
          .getOpenType(), paramMBeanAttributeInfo
          .isReadable(), paramMBeanAttributeInfo
          .isWritable(), paramMBeanAttributeInfo
          .isIs(), paramMBeanAttributeInfo
          .getDescriptor());
    } 
    return new MBeanAttributeInfo(paramMBeanAttributeInfo.getName(), paramMBeanAttributeInfo
        .getType(), paramString, paramMBeanAttributeInfo
  ...
  private static OpenMBeanParameterInfo[] paramsToOpenParams(MBeanParameterInfo[] paramArrayOfMBeanParameterInfo) {
    ...
    return (OpenMBeanParameterInfo[])arrayOfOpenMBeanParameterInfoSupport;
  }
    ...
    for (byte b = 0; b < i; b++) {
      ...
      if (arrayOfMBeanParameterInfo1 != null) {
        ...
        for (byte b1 = 0; b1 < j; b1++) {
          ...
          arrayOfMBeanParameterInfo2[b1] = customize(mBeanParameterInfo, 
              getParameterName(mBeanConstructorInfo, mBeanParameterInfo, b1), 
              getDescription(mBeanConstructorInfo, mBeanParameterInfo, b1));
        } 
      } else {
        arrayOfMBeanParameterInfo2 = null;
      } 
      arrayOfMBeanConstructorInfo2[b] = 
        customize(mBeanConstructorInfo, getDescription(mBeanConstructorInfo), arrayOfMBeanParameterInfo2);
    } 
    ...
    for (byte b = 0; b < i; b++) {
      ...
      if (arrayOfMBeanParameterInfo1 != null) {
        ...
        for (byte b1 = 0; b1 < j; b1++) {
          MBeanParameterInfo mBeanParameterInfo = arrayOfMBeanParameterInfo1[b1];
          arrayOfMBeanParameterInfo2[b1] = customize(mBeanParameterInfo, 
              getParameterName(mBeanOperationInfo, mBeanParameterInfo, b1), 
              getDescription(mBeanOperationInfo, mBeanParameterInfo, b1));
        } 
      } else {
        arrayOfMBeanParameterInfo2 = null;
      } 
  ...
  static boolean immutableInfo(Class<? extends StandardMBean> paramClass) {
    ...
    synchronized (mbeanInfoSafeMap) {
      Boolean bool = mbeanInfoSafeMap.get(paramClass);
      if (bool == null) {
        try {
          MBeanInfoSafeAction mBeanInfoSafeAction = new MBeanInfoSafeAction(paramClass);
          bool = AccessController.<Boolean>doPrivileged(mBeanInfoSafeAction);
  ...
  static boolean overrides(Class<?> paramClass1, Class<?> paramClass2, String paramString, Class<?>... paramVarArgs) {
    for (Class<?> clazz = paramClass1; clazz != paramClass2; clazz = clazz.getSuperclass()) {
      try {
  ...
  private static class StandardMBean {}
}
