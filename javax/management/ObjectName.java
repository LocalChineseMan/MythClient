...
public class ObjectName implements Comparable<ObjectName>, QueryExp {
  ...
  private static class Property {
    ...
    void setKeyIndex(int param1Int) {
      this._key_index = param1Int;
    }
    
    String getKeyString(String param1String) {
      return param1String.substring(this._key_index, this._key_index + this._key_length);
    }
  ...
  private void construct(String paramString) throws MalformedObjectNameException {
    ...
    while (b1 < i) {
      ...
      b3++;
      if (!bool2) {
        property = new Property(b4, k, m);
      } else {
        this._property_value_pattern = true;
        property = new PatternProperty(b4, k, m);
      } 
      String str = paramString.substring(b4, b4 + k);
      ...
      addProperty(property, b2, (Map)hashMap, str);
      b2++;
      b1 = b3;
    } 
    setCanonicalName(arrayOfChar1, arrayOfChar2, arrayOfString, (Map)hashMap, j, b2);
  }
  ...
  private void construct(String paramString, Map<String, String> paramMap) throws MalformedObjectNameException {
    ...
    for (Map.Entry<String, String> entry : paramMap.entrySet()) {
      ...
      if (!bool) {
        property = new Property(k, str1.length(), str2.length());
      } else {
        this._property_value_pattern = true;
        property = new PatternProperty(k, str1.length(), str2.length());
      } 
      addProperty(property, b, (Map)hashMap, str1);
      b++;
    } 
    ...
    setCanonicalName(arrayOfChar1, arrayOfChar2, arrayOfString, (Map)hashMap, this._domain_length + 1, this._kp_array.length);
  }
  ...
  private void setCanonicalName(char[] paramArrayOfchar1, char[] paramArrayOfchar2, String[] paramArrayOfString, Map<String, Property> paramMap, int paramInt1, int paramInt2) {
    if (this._kp_array != _Empty_property_array) {
      ...
      for (i = 0; i < paramInt2; i++)
        this._ca_array[i] = paramMap.get(paramArrayOfString[i]); 
      i = paramInt2 - 1;
      for (byte b = 0; b <= i; b++) {
  ...
  private static boolean checkValue(String paramString) throws MalformedObjectNameException {
    ...
    int j = arrayOfInt[0];
    boolean bool = (arrayOfInt[1] == 1) ? true : false;
    if (j < i)
  ...
  public ObjectName(String paramString1, String paramString2, String paramString3) throws MalformedObjectNameException {
    Map<String, String> map = Collections.singletonMap(paramString2, paramString3);
  ...
  public String getKeyProperty(String paramString) {
    return _getKeyPropertyList().get(paramString);
  }
    ...
    synchronized (this) {
      if (this._propertyList == null) {
        ...
        for (int j = i - 1; j >= 0; j--) {
          ...
          this._propertyList.put(property.getKeyString(this._canonicalName), property
              .getValueString(this._canonicalName));
        } 
      } 
    } 
  ...
  public boolean apply(ObjectName paramObjectName) {
    ...
    if (paramObjectName._domain_pattern || paramObjectName._property_list_pattern || paramObjectName._property_value_pattern)
      return false; 
    if (!this._domain_pattern && !this._property_list_pattern && !this._property_value_pattern)
      return this._canonicalName.equals(paramObjectName._canonicalName); 
  ...
  private final boolean matchKeys(ObjectName paramObjectName) {
    ...
    if (this._property_value_pattern || this._property_list_pattern) {
      Map<String, String> map = paramObjectName._getKeyPropertyList();
