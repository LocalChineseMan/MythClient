...
public class ArrayType<T> extends OpenType<T> {
  ...
  private static final Object[][] PRIMITIVE_ARRAY_TYPES = new Object[][] { { Boolean.class
        .getName(), boolean.class.getName(), "Z", SimpleType.BOOLEAN }, { Character.class
        .getName(), char.class.getName(), "C", SimpleType.CHARACTER }, { Byte.class
        .getName(), byte.class.getName(), "B", SimpleType.BYTE }, { Short.class
        .getName(), short.class.getName(), "S", SimpleType.SHORT }, { Integer.class
        .getName(), int.class.getName(), "I", SimpleType.INTEGER }, { Long.class
        .getName(), long.class.getName(), "J", SimpleType.LONG }, { Float.class
        .getName(), float.class.getName(), "F", SimpleType.FLOAT }, { Double.class
        .getName(), double.class.getName(), "D", SimpleType.DOUBLE } };
  ...
  public ArrayType(int paramInt, OpenType<?> paramOpenType) throws OpenDataException {
    super(buildArrayClassName(paramInt, paramOpenType), 
        buildArrayClassName(paramInt, paramOpenType), 
        buildArrayDescription(paramInt, paramOpenType));
  ...
  public ArrayType(SimpleType<?> paramSimpleType, boolean paramBoolean) throws OpenDataException {
    super(buildArrayClassName(1, paramSimpleType, paramBoolean), 
        buildArrayClassName(1, paramSimpleType, paramBoolean), 
        buildArrayDescription(1, paramSimpleType, paramBoolean), true);
    this.dimension = 1;
  ...
  public boolean isValue(Object paramObject) {
    ...
    if (this.elementType.getClassName().equals(TabularData.class.getName()) || this.elementType
      ...
      Class clazz1 = (Class)(bool ? TabularData.class : CompositeData.class);
      Class<?> clazz2 = Array.newInstance(clazz1, arrayOfInt).getClass();
      if (!clazz2.isAssignableFrom(clazz))
        return false; 
      if (!checkElementsType((Object[])paramObject, this.dimension))
        return false; 
      return true;
    } 
    return false;
  ...
  boolean isAssignableFrom(OpenType<?> paramOpenType) {
    if (!(paramOpenType instanceof ArrayType))
      return false; 
    ArrayType arrayType = (ArrayType)paramOpenType;
    return (arrayType.getDimension() == getDimension() && arrayType
  ...
  public String toString() {
    if (this.myToString == null)
      this
        .myToString = getClass().getName() + "(name=" + getTypeName() + ",dimension=" + this.dimension + ",elementType=" + this.elementType + ",primitiveArray=" + this.primitiveArray + ")"; 
