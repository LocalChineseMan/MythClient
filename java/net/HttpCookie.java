...
public final class HttpCookie implements Cloneable {
  ...
  private static List<HttpCookie> parse(String paramString, boolean paramBoolean) {
    ...
    if (i == 0) {
      ...
      List<String> list = splitMultiCookies(paramString);
      for (String str : list) {
  ...
  public static boolean domainMatches(String paramString1, String paramString2) {
    ...
    if (!bool && (i == -1 || i == paramString1
      
      .length() - 1))
      return false; 
    int j = paramString2.indexOf('.');
    if (j == -1 && (bool || paramString1
      
      .equalsIgnoreCase(paramString2 + ".local")))
      return true; 
  ...
  public boolean equals(Object paramObject) {
    ...
    if (!(paramObject instanceof HttpCookie))
      return false; 
    HttpCookie httpCookie = (HttpCookie)paramObject;
    return (equalsIgnoreCase(getName(), httpCookie.getName()) && 
      equalsIgnoreCase(getDomain(), httpCookie.getDomain()) && 
      Objects.equals(getPath(), httpCookie.getPath()));
  }
  
  public int hashCode() {
    int i = this.name.toLowerCase().hashCode();
    byte b1 = (this.domain != null) ? this.domain.toLowerCase().hashCode() : 0;
    byte b2 = (this.path != null) ? this.path.hashCode() : 0;
    return i + b1 + b2;
  }
  ...
  static {
    ...
    assignors.put("discard", new CookieAttributeAssignor() {
          public void assign(HttpCookie param1HttpCookie, String param1String1, String param1String2) {
            param1HttpCookie.setDiscard(true);
          }
        });
    ...
    assignors.put("secure", new CookieAttributeAssignor() {
          public void assign(HttpCookie param1HttpCookie, String param1String1, String param1String2) {
            param1HttpCookie.setSecure(true);
          }
        });
    assignors.put("httponly", new CookieAttributeAssignor() {
          public void assign(HttpCookie param1HttpCookie, String param1String1, String param1String2) {
            param1HttpCookie.setHttpOnly(true);
          }
        });
    ...
    SharedSecrets.setJavaNetHttpCookieAccess(new JavaNetHttpCookieAccess() {
          public List<HttpCookie> parse(String param1String) {
            return HttpCookie.parse(param1String, true);
          }
          
          public String header(HttpCookie param1HttpCookie) {
            return param1HttpCookie.header;
          }
        });
  }
  ...
  private long expiryDate2DeltaSeconds(String paramString) {
    ...
    for (byte b = 0; b < COOKIE_DATE_FORMATS.length; b++) {
  ...
  private static String stripOffSurroundingQuote(String paramString) {
    if (paramString != null && paramString.length() > 2 && paramString
      .charAt(0) == '"' && paramString.charAt(paramString.length() - 1) == '"')
  ...
  private static boolean startsWithIgnoreCase(String paramString1, String paramString2) {
    if (paramString1 == null || paramString2 == null)
      return false; 
    if (paramString1.length() >= paramString2.length() && paramString2
      .equalsIgnoreCase(paramString1.substring(0, paramString2.length())))
      return true; 
    return false;
  }
  ...
  private static List<String> splitMultiCookies(String paramString) {
    ...
    for (byte b2 = 0; b2 < paramString.length(); b2++) {
      ...
      if (c == '"')
        b1++; 
      if (c == ',' && b1 % 2 == 0) {
        arrayList.add(paramString.substring(i, b2));
        i = b2 + 1;
      } 
    } 
