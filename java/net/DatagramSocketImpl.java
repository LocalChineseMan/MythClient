...
public abstract class DatagramSocketImpl implements SocketOptions {
  ...
  <T> void setOption(SocketOption<T> paramSocketOption, T paramT) throws IOException {
    if (paramSocketOption == StandardSocketOptions.SO_SNDBUF) {
      ...
      setOption(3, paramT);
    } else if (paramSocketOption == StandardSocketOptions.IP_MULTICAST_IF && 
      getDatagramSocket() instanceof MulticastSocket) {
      setOption(31, paramT);
    } else if (paramSocketOption == StandardSocketOptions.IP_MULTICAST_TTL && 
      getDatagramSocket() instanceof MulticastSocket) {
      if (!(paramT instanceof Integer))
        throw new IllegalArgumentException("not an integer"); 
      setTimeToLive(((Integer)paramT).intValue());
    } else if (paramSocketOption == StandardSocketOptions.IP_MULTICAST_LOOP && 
      getDatagramSocket() instanceof MulticastSocket) {
      setOption(18, paramT);
    } else {
  ...
  <T> T getOption(SocketOption<T> paramSocketOption) throws IOException {
    ...
    if (paramSocketOption == StandardSocketOptions.IP_MULTICAST_LOOP && 
      getDatagramSocket() instanceof MulticastSocket)
