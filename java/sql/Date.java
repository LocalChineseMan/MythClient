...
public class Date extends Date {
  ...
  public static Date valueOf(String paramString) {
    ...
    if (i > 0 && j > 0 && j < paramString.length() - 1) {
      ...
      if (str1.length() == 4 && str2
        .length() >= 1 && str2.length() <= 2 && str3
        .length() >= 1 && str3.length() <= 2) {
  ...
  public static Date valueOf(LocalDate paramLocalDate) {
    return new Date(paramLocalDate.getYear() - 1900, paramLocalDate.getMonthValue() - 1, paramLocalDate
        .getDayOfMonth());
  }
