...
class TypeConvertingMethodAdapter extends MethodVisitor {
  ...
  private static int hashWrapperName(String paramString) {
    if (paramString.length() < 3)
      return 0; 
    return (3 * paramString.charAt(1) + paramString.charAt(2)) % 16;
  }
  ...
  private Wrapper wrapperOrNullFromDescriptor(String paramString) {
    ...
    if (wrapper == null || wrapper.wrapperSimpleName().equals(str))
      return wrapper; 
    return null;
  }
  ...
  void box(Wrapper paramWrapper) {
    visitMethodInsn(184, 
        wrapperName(paramWrapper), "valueOf", 
        
        boxingDescriptor(paramWrapper), false);
  }
  
  void unbox(String paramString, Wrapper paramWrapper) {
    visitMethodInsn(182, paramString, 
        
        unboxMethod(paramWrapper), 
        unboxingDescriptor(paramWrapper), false);
  }
  ...
  void convertType(Class<?> paramClass1, Class<?> paramClass2, Class<?> paramClass3) {
    ...
    if (paramClass1.isPrimitive()) {
      ...
      String str2, str1 = BytecodeDescriptor.unparse(paramClass1);
