...
class InvokerBytecodeGenerator {
  ...
  private InvokerBytecodeGenerator(String paramString, LambdaForm paramLambdaForm, MethodType paramMethodType) {
    ...
    byte b;
    int i;
    for (b = 0, i = 0; b < this.localsMap.length; b++) {
      this.localsMap[b] = i;
  ...
  static void maybeDump(String paramString, byte[] paramArrayOfbyte) {
    if (MethodHandleStatics.DUMP_CLASS_FILES)
      AccessController.doPrivileged((PrivilegedAction<?>)new Object(paramString, paramArrayOfbyte)); 
  }
  
  private static String makeDumpableClassName(String paramString) {
    Integer integer;
    synchronized (DUMP_CLASS_FILES_COUNTERS) {
      integer = DUMP_CLASS_FILES_COUNTERS.get(paramString);
      if (integer == null)
  ...
  private static Class<?>[] STATICALLY_INVOCABLE_PACKAGES = new Class[] { Object.class, Arrays.class, Unsafe.class };
  
  static boolean isStaticallyInvocable(LambdaForm.Name paramName) {
    return isStaticallyInvocable(paramName.function.member());
  }
  ...
  static boolean isStaticallyInvocable(MemberName paramMemberName) {
    ...
    if (!paramMemberName.isPrivate() && VerifyAccess.isSamePackage(MethodHandle.class, clazz))
      return true; 
    if (paramMemberName.isPublic() && isStaticallyNameable(clazz))
      return true; 
    return false;
  }
  ...
  void emitStaticInvoke(LambdaForm.Name paramName) {
    emitStaticInvoke(paramName.function.member(), paramName);
  }
  ...
  void emitStaticInvoke(MemberName paramMemberName, LambdaForm.Name paramName) {
    ...
    if (paramMemberName.isMethod()) {
      ...
      this.mv.visitMethodInsn(refKindOpcode(b), str1, str2, str, paramMemberName
  ...
  private boolean memberRefersTo(MemberName paramMemberName, Class<?> paramClass, String paramString) {
    return (paramMemberName != null && paramMemberName
      .getDeclaringClass() == paramClass && paramMemberName
      .getName().equals(paramString));
  }
  
  private boolean nameRefersTo(LambdaForm.Name paramName, Class<?> paramClass, String paramString) {
    return (paramName.function != null && 
      memberRefersTo(paramName.function.member(), paramClass, paramString));
  }
  ...
  private boolean isInvokeBasic(LambdaForm.Name paramName) {
    ...
    return (memberRefersTo(memberName, MethodHandle.class, "invokeBasic") && 
      !memberName.isPublic() && !memberName.isStatic());
  }
  ...
  private boolean isLinkerMethodInvoke(LambdaForm.Name paramName) {
    ...
    return (memberName != null && memberName
      .getDeclaringClass() == MethodHandle.class && 
      !memberName.isPublic() && memberName.isStatic() && memberName
      .getName().startsWith("linkTo"));
  }
  ...
  private boolean isSelectAlternative(int paramInt) {
    ...
    return (nameRefersTo(name1, MethodHandleImpl.class, "selectAlternative") && 
      isInvokeBasic(name2) && name2
      .lastUseIndex(name1) == 0 && this.lambdaForm
      .lastUseIndex(name1) == paramInt + 1);
  }
  ...
  private boolean isGuardWithCatch(int paramInt) {
    ...
    return (nameRefersTo(name2, MethodHandleImpl.class, "guardWithCatch") && 
      isInvokeBasic(name1) && 
      isInvokeBasic(name3) && name2
      .lastUseIndex(name1) == 3 && this.lambdaForm
      .lastUseIndex(name1) == paramInt + 1 && name3
      .lastUseIndex(name2) == 1 && this.lambdaForm
      .lastUseIndex(name2) == paramInt + 2);
  }
