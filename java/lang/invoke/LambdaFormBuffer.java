...
final class LambdaFormBuffer {
  ...
  LambdaForm.Name[] nameArray() {
    return Arrays.<LambdaForm.Name>copyOf(this.names, this.length);
  }
  ...
  private boolean verifyArity() {
    ...
    if (this.resultName != null) {
      ...
      assert i >= 0 : "not found: " + this.resultName.exprString() + Arrays.asList((T[])this.names);
  ...
  private boolean verifyFirstChange() {
    ...
    for (byte b = 0; b < this.length; b++) {
      if (this.names[b] != this.originalNames[b]) {
        assert this.firstChange == b : Arrays.asList((T[])new Object[] { Integer.valueOf(this.firstChange), Integer.valueOf(b), this.originalNames[b].exprString(), Arrays.asList(this.names) });
        return true;
      } 
    } 
    assert this.firstChange == this.length : Arrays.asList((T[])new Object[] { Integer.valueOf(this.firstChange), Arrays.asList(this.names) });
    return true;
  }
  ...
  void growNames(int paramInt1, int paramInt2) {
    ...
    if (k == 0 || j > this.names.length) {
      this.names = Arrays.<LambdaForm.Name>copyOf(this.names, (this.names.length + paramInt2) * 5 / 4);
      if (k == 0) {
    ...
    if (this.originalNames != null && this.originalNames.length < this.names.length) {
      this.originalNames = Arrays.<LambdaForm.Name>copyOf(this.originalNames, this.names.length);
      if (k == 1) {
        this.flags = (byte)(this.flags + 1);
        k++;
        assert ownedCount() == k;
    ...
    Arrays.fill((Object[])this.names, paramInt1, m, (Object)null);
    if (this.originalNames != null) {
      System.arraycopy(this.originalNames, paramInt1, this.originalNames, m, n);
      Arrays.fill((Object[])this.originalNames, paramInt1, m, (Object)null);
    } 
    this.length = j;
  ...
  private void clearDuplicatesAndNulls() {
      ...
      for (LambdaForm.Name name : this.dups) {
        for (int k = this.firstChange; k < this.length; k++) {
          if (this.names[k] == name && this.originalNames[k] != name) {
            this.names[k] = null;
            assert Arrays.<LambdaForm.Name>asList(this.names).contains(name);
            break;
          } 
        } 
      } 
    ...
    if (this.length < i)
      Arrays.fill((Object[])this.names, this.length, i, (Object)null); 
    assert !Arrays.<LambdaForm.Name>asList(this.names).subList(0, this.length).contains(null);
  }
  ...
  void startEdit() {
    ...
    if (arrayOfName2 != null && arrayOfName2.length >= this.length) {
      ...
      this.names = Arrays.<LambdaForm.Name>copyOf(arrayOfName1, Math.max(this.length + 2, arrayOfName1.length));
      if (i < 2)
        this.flags = (byte)(this.flags + 1); 
  ...
  LambdaForm endEdit() {
    ...
    if (this.firstChange < this.arity) {
      ...
      byte b = 0;
      for (int k = this.firstChange; k < this.arity; k++) {
        LambdaForm.Name name = this.names[k];
        if (name.isParam()) {
          this.names[j++] = name;
        } else {
          arrayOfName[b++] = name;
      ...
      assert b == this.arity - j;
      System.arraycopy(arrayOfName, 0, this.names, j, b);
      this.arity -= b;
    } 
    assert verifyArity();
    return lambdaForm();
  ...
  private LambdaForm.Name[] copyNamesInto(LambdaForm.Name[] paramArrayOfName) {
    ...
    Arrays.fill((Object[])paramArrayOfName, this.length, paramArrayOfName.length, (Object)null);
