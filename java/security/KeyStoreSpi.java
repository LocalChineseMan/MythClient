...
public abstract class KeyStoreSpi {
    ...
    if (paramProtectionParameter == null) {
      if (engineIsCertificateEntry(paramString))
        return new KeyStore.TrustedCertificateEntry(
            engineGetCertificate(paramString)); 
  ...
  public void engineSetEntry(String paramString, KeyStore.Entry paramEntry, KeyStore.ProtectionParameter paramProtectionParameter) throws KeyStoreException {
    ...
    if (paramEntry instanceof KeyStore.PrivateKeyEntry) {
      ...
      engineSetKeyEntry(paramString, ((KeyStore.PrivateKeyEntry)paramEntry)
          .getPrivateKey(), passwordProtection
          .getPassword(), ((KeyStore.PrivateKeyEntry)paramEntry)
          .getCertificateChain());
      return;
    } 
    if (paramEntry instanceof KeyStore.SecretKeyEntry) {
      ...
      engineSetKeyEntry(paramString, ((KeyStore.SecretKeyEntry)paramEntry)
          .getSecretKey(), passwordProtection
          .getPassword(), (Certificate[])null);
      return;
    } 
    throw new KeyStoreException("unsupported entry type: " + paramEntry
        .getClass().getName());
  }
  ...
  public boolean engineEntryInstanceOf(String paramString, Class<? extends KeyStore.Entry> paramClass) {
    ...
    if (paramClass == KeyStore.PrivateKeyEntry.class)
      return (engineIsKeyEntry(paramString) && 
        engineGetCertificate(paramString) != null); 
