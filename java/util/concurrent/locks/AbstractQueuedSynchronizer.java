...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements Serializable {
    ...
    while (true) {
      ...
      if (node != null && node != this.tail) {
        ...
        if (i == -1) {
          ...
          !compareAndSetWaitStatus(node, 0, -3)) {
          continue;
        } 
      } 
      if (node == this.head)
  ...
  private void cancelAcquire(Node paramNode) {
    ...
    if (paramNode == this.tail && compareAndSetTail(paramNode, node1)) {
      ...
      int i;
      if (node1 != this.head && ((i = node1.waitStatus) == -1 || (i <= 0 && 
        
        compareAndSetWaitStatus(node1, i, -1))) && node1.thread != null) {
        Node node = paramNode.next;
        if (node != null && node.waitStatus <= 0)
  ...
  final boolean acquireQueued(Node paramNode, int paramInt) {
    ...
    try {
      ...
      while (true) {
        ...
        if (shouldParkAfterFailedAcquire(node, paramNode) && 
  ...
  private void doAcquireInterruptibly(int paramInt) throws InterruptedException {
    ...
    try {
      while (true) {
        ...
        if (shouldParkAfterFailedAcquire(node1, node) && 
          parkAndCheckInterrupt())
  ...
  public final void acquire(int paramInt) {
    if (!tryAcquire(paramInt) && 
      acquireQueued(addWaiter(Node.EXCLUSIVE), paramInt))
      selfInterrupt(); 
  }
  ...
  public final boolean tryAcquireNanos(int paramInt, long paramLong) throws InterruptedException {
    if (Thread.interrupted())
      throw new InterruptedException(); 
    return (tryAcquire(paramInt) || 
      doAcquireNanos(paramInt, paramLong));
  }
  ...
  public final boolean tryAcquireSharedNanos(int paramInt, long paramLong) throws InterruptedException {
    if (Thread.interrupted())
      throw new InterruptedException(); 
    return (tryAcquireShared(paramInt) >= 0 || 
      doAcquireSharedNanos(paramInt, paramLong));
  }
  ...
  final boolean apparentlyFirstQueuedIsExclusive() {
    Node node1;
    Node node2;
    return ((node1 = this.head) != null && (node2 = node1.next) != null && 
      
      !node2.isShared() && node2.thread != null);
  }
  ...
  public final boolean hasQueuedPredecessors() {
    ...
    Node node3;
    return (node2 != node1 && ((node3 = node2.next) == null || node3.thread != 
  ...
  public class ConditionObject implements Condition, Serializable {
    ...
    private int checkInterruptWhileWaiting(AbstractQueuedSynchronizer.Node param1Node) {
      return Thread.interrupted() ? (
        AbstractQueuedSynchronizer.this.transferAfterCancelledWait(param1Node) ? -1 : 1) : 0;
    }
    ...
    final boolean isOwnedBy(AbstractQueuedSynchronizer param1AbstractQueuedSynchronizer) {
      return (param1AbstractQueuedSynchronizer == AbstractQueuedSynchronizer.this);
    }
