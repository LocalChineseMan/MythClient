...
public class LogManager {
  ...
  static {
    manager = AccessController.<LogManager>doPrivileged(new PrivilegedAction<LogManager>() {
  ...
  final void ensureLogManagerInitialized() {
    ...
    if (this.initializationDone || logManager != manager)
      return; 
    synchronized (this) {
      boolean bool = (this.initializedCalled == true) ? true : false;
      ...
      try {
        AccessController.doPrivileged(new PrivilegedAction() {
              public Object run() {
                ...
                owner.getClass();
                owner.rootLogger = new LogManager.RootLogger();
  ...
  public void addPropertyChangeListener(PropertyChangeListener paramPropertyChangeListener) throws SecurityException {
    PropertyChangeListener propertyChangeListener = Objects.<PropertyChangeListener>requireNonNull(paramPropertyChangeListener);
    checkPermission();
    synchronized (this.listenerMap) {
      Integer integer = this.listenerMap.get(propertyChangeListener);
    ...
    if (securityManager != null && javaAWTAccess != null) {
      ...
      if (object != null)
        synchronized (javaAWTAccess) {
          ...
          loggerContext = this.contextsMap.get(object);
  ...
  private List<LoggerContext> contexts() {
    ArrayList<LoggerContext> arrayList = new ArrayList();
  ...
  Logger demandSystemLogger(String paramString1, String paramString2) {
    Logger logger2, logger1 = getSystemContext().demandLogger(paramString1, paramString2);
    do {
      if (addLogger(logger1)) {
        logger2 = logger1;
      } else {
    ...
    if (logger2 != logger1 && (logger1.accessCheckedHandlers()).length == 0) {
      ...
      AccessController.doPrivileged((PrivilegedAction<?>)new Object(this, logger, logger1));
    } 
    return logger1;
  }
  ...
  class LoggerContext {
    ...
    private LoggerContext() {
      this.root = new LogManager.LogNode(null, this);
    }
    
    final boolean requiresDefaultLoggers() {
      boolean bool = (getOwner() == LogManager.manager) ? true : false;
      if (bool)
    ...
    synchronized Logger findLogger(String param1String) {
      ensureInitialized();
      LogManager.LoggerWeakRef loggerWeakRef = this.namedLoggers.get(param1String);
    ...
    boolean addLocalLogger(Logger param1Logger) {
      return addLocalLogger(param1Logger, requiresDefaultLoggers());
    }
    
    synchronized boolean addLocalLogger(Logger param1Logger, boolean param1Boolean) {
      if (param1Boolean)
        ensureAllDefaultLoggers(param1Logger); 
      String str = param1Logger.getName();
      if (str == null)
        throw new NullPointerException(); 
      LogManager.LoggerWeakRef loggerWeakRef = this.namedLoggers.get(str);
      if (loggerWeakRef != null)
      ...
      logManager.getClass();
      loggerWeakRef = new LogManager.LoggerWeakRef(param1Logger);
    ...
    synchronized void removeLoggerRef(String param1String, LogManager.LoggerWeakRef param1LoggerWeakRef) {
      this.namedLoggers.remove(param1String, param1LoggerWeakRef);
    }
    ...
    LogManager.LogNode getNode(String param1String) {
      if (param1String == null || param1String.equals(""))
        return this.root; 
      LogManager.LogNode logNode = this.root;
      while (param1String.length() > 0) {
        String str;
        ...
        LogManager.LogNode logNode1 = logNode.children.get(str);
        if (logNode1 == null) {
  ...
  private void loadLoggerHandlers(final Logger logger, String paramString1, final String handlersPropertyName) {
    AccessController.doPrivileged(new PrivilegedAction() {
          public Object run() {
  ...
  final class LoggerWeakRef extends WeakReference<Logger> {
    ...
    LoggerWeakRef(Logger param1Logger) {
      super(param1Logger, LogManager.this.loggerRefQueue);
    ...
    void setNode(LogManager.LogNode param1LogNode) {
      this.node = param1LogNode;
    }
    
    void setParentRef(WeakReference<Logger> param1WeakReference) {
      this.parentRef = param1WeakReference;
    }
  }
  ...
  private static void doSetLevel(Logger paramLogger, Level paramLevel) {
    SecurityManager securityManager = System.getSecurityManager();
    if (securityManager == null) {
      paramLogger.setLevel(paramLevel);
      return;
    } 
    AccessController.doPrivileged((PrivilegedAction<?>)new Object(paramLogger, paramLevel));
  }
  
  private static void doSetParent(Logger paramLogger1, Logger paramLogger2) {
    SecurityManager securityManager = System.getSecurityManager();
    if (securityManager == null) {
      paramLogger1.setParent(paramLogger2);
      return;
    } 
    AccessController.doPrivileged((PrivilegedAction<?>)new Object(paramLogger1, paramLogger2));
  }
  ...
  private static class LogNode {
    ...
    void walkAndSetParent(Logger param1Logger) {
      if (this.children == null)
        return; 
      Iterator<LogNode> iterator = this.children.values().iterator();
