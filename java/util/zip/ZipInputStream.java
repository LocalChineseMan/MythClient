...
public class ZipInputStream extends InflaterInputStream implements ZipConstants {
  ...
  private ZipEntry readLOC() throws IOException {
    ...
    ZipEntry zipEntry = createZipEntry(((this.flag & 0x800) != 0) ? this.zc
        .toStringUTF8(this.b, i) : this.zc
        .toString(this.b, i));
    if ((this.flag & 0x1) == 1)
      throw new ZipException("encrypted ZIP entry not supported"); 
    zipEntry.method = ZipUtils.get16(this.tmpbuf, 8);
    zipEntry.time = ZipUtils.dosToJavaTime(ZipUtils.get32(this.tmpbuf, 10));
    if ((this.flag & 0x8) == 8) {
  ...
  private void readEnd(ZipEntry paramZipEntry) throws IOException {
    ...
    if ((this.flag & 0x8) == 8)
      if (this.inf.getBytesWritten() > 4294967295L || this.inf
        .getBytesRead() > 4294967295L) {
    ...
    if (paramZipEntry.size != this.inf.getBytesWritten())
      throw new ZipException("invalid entry size (expected " + paramZipEntry.size + " but got " + this.inf
          
          .getBytesWritten() + " bytes)"); 
    if (paramZipEntry.csize != this.inf.getBytesRead())
      throw new ZipException("invalid entry compressed size (expected " + paramZipEntry.csize + " but got " + this.inf
          
          .getBytesRead() + " bytes)"); 
    if (paramZipEntry.crc != this.crc.getValue())
      throw new ZipException("invalid entry CRC (expected 0x" + 
          Long.toHexString(paramZipEntry.crc) + " but got 0x" + 
