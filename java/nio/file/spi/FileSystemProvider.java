...
public abstract class FileSystemProvider {
  ...
  private static List<FileSystemProvider> loadInstalledProviders() {
    ArrayList<FileSystemProvider> arrayList = new ArrayList();
    ...
    if (installedProviders == null) {
      ...
      synchronized (lock) {
        if (installedProviders == null) {
          ...
          List<FileSystemProvider> list = AccessController.<List>doPrivileged((PrivilegedAction<List>)new Object());
  ...
  public OutputStream newOutputStream(Path paramPath, OpenOption... paramVarArgs) throws IOException {
    ...
    HashSet<StandardOpenOption> hashSet = new HashSet(i + 3);
    if (i == 0) {
    ...
    return Channels.newOutputStream(newByteChannel(paramPath, (Set)hashSet, (FileAttribute<?>[])new FileAttribute[0]));
  }
