...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  enum Strength {
    STRONG {
      <K, V> LocalCache.ValueReference<K, V> referenceValue(LocalCache.Segment<K, V> segment, LocalCache.ReferenceEntry<K, V> entry, V value, int weight) {
        return (weight == 1) ? new LocalCache.StrongValueReference<K, V>(value) : (LocalCache.ValueReference<K, V>)new LocalCache.WeightedStrongValueReference(value, weight);
      }
    ...
    SOFT {
      <K, V> LocalCache.ValueReference<K, V> referenceValue(LocalCache.Segment<K, V> segment, LocalCache.ReferenceEntry<K, V> entry, V value, int weight) {
        return (weight == 1) ? (LocalCache.ValueReference<K, V>)new LocalCache.SoftValueReference(segment.valueReferenceQueue, value, entry) : (LocalCache.ValueReference<K, V>)new LocalCache.WeightedSoftValueReference(segment.valueReferenceQueue, value, entry, weight);
      }
    ...
    WEAK {
      <K, V> LocalCache.ValueReference<K, V> referenceValue(LocalCache.Segment<K, V> segment, LocalCache.ReferenceEntry<K, V> entry, V value, int weight) {
        return (weight == 1) ? (LocalCache.ValueReference<K, V>)new LocalCache.WeakValueReference(segment.valueReferenceQueue, value, entry) : (LocalCache.ValueReference<K, V>)new LocalCache.WeightedWeakValueReference(segment.valueReferenceQueue, value, entry, weight);
      }
  ...
  enum EntryFactory {
    ...
    STRONG_ACCESS_WRITE {
      <K, V> LocalCache.ReferenceEntry<K, V> newEntry(LocalCache.Segment<K, V> segment, K key, int hash, @Nullable LocalCache.ReferenceEntry<K, V> next) {
        return (LocalCache.ReferenceEntry<K, V>)new LocalCache.StrongAccessWriteEntry(key, hash, next);
      }
    ...
    WEAK {
      <K, V> LocalCache.ReferenceEntry<K, V> newEntry(LocalCache.Segment<K, V> segment, K key, int hash, @Nullable LocalCache.ReferenceEntry<K, V> next) {
        return (LocalCache.ReferenceEntry<K, V>)new LocalCache.WeakEntry(segment.keyReferenceQueue, key, hash, next);
      }
    },
    WEAK_ACCESS {
      <K, V> LocalCache.ReferenceEntry<K, V> newEntry(LocalCache.Segment<K, V> segment, K key, int hash, @Nullable LocalCache.ReferenceEntry<K, V> next) {
        return (LocalCache.ReferenceEntry<K, V>)new LocalCache.WeakAccessEntry(segment.keyReferenceQueue, key, hash, next);
      }
    ...
    WEAK_WRITE {
      <K, V> LocalCache.ReferenceEntry<K, V> newEntry(LocalCache.Segment<K, V> segment, K key, int hash, @Nullable LocalCache.ReferenceEntry<K, V> next) {
        return (LocalCache.ReferenceEntry<K, V>)new LocalCache.WeakWriteEntry(segment.keyReferenceQueue, key, hash, next);
      }
    ...
    WEAK_ACCESS_WRITE {
      <K, V> LocalCache.ReferenceEntry<K, V> newEntry(LocalCache.Segment<K, V> segment, K key, int hash, @Nullable LocalCache.ReferenceEntry<K, V> next) {
        return (LocalCache.ReferenceEntry<K, V>)new LocalCache.WeakAccessWriteEntry(segment.keyReferenceQueue, key, hash, next);
      }
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight, AbstractCache$StatsCounter statsCounter) {
      ...
      this.writeQueue = map.usesWriteQueue() ? (Queue<LocalCache.ReferenceEntry<K, V>>)new LocalCache.WriteQueue() : LocalCache.<LocalCache.ReferenceEntry<K, V>>discardingQueue();
      this.accessQueue = map.usesAccessQueue() ? (Queue<LocalCache.ReferenceEntry<K, V>>)new LocalCache.AccessQueue() : LocalCache.<LocalCache.ReferenceEntry<K, V>>discardingQueue();
    }
      ...
      try {
        ...
        if (createNewEntry) {
          ...
          if (e == null) {
            ...
            e.setValueReference((LocalCache.ValueReference<K, V>)loadingValueReference);
            table.set(index, e);
          } else {
            e.setValueReference((LocalCache.ValueReference<K, V>)loadingValueReference);
          } 
    ...
    ListenableFuture<V> loadAsync(K key, int hash, LocalCache.LoadingValueReference<K, V> loadingValueReference, CacheLoader<? super K, V> loader) {
      ListenableFuture<V> loadingFuture = loadingValueReference.loadFuture(key, loader);
      loadingFuture.addListener((Runnable)new Object(this, key, hash, loadingValueReference, loadingFuture), (Executor)LocalCache.sameThreadExecutor);
      ...
      try {
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
            ...
            e.setValueReference((LocalCache.ValueReference<K, V>)loadingValueReference1);
          ...
          if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
            ...
            if (oldValueReference == valueReference || (entryValue == null && valueReference != LocalCache.UNSET)) {
              ...
              if (oldValueReference.isActive()) {
                ...
                enqueueNotification(key, hash, (LocalCache.ValueReference<K, V>)oldValueReference, cause);
                newCount--;
              } 
              setValue(e, key, newValue, now);
              this.count = newCount;
            ...
            LocalCache.WeightedStrongValueReference weightedStrongValueReference = new LocalCache.WeightedStrongValueReference(newValue, 0);
            enqueueNotification(key, hash, (LocalCache.ValueReference<K, V>)weightedStrongValueReference, RemovalCause.REPLACED);
            return false;
          } 
        } 
        this.modCount++;
  ...
  static class LoadingValueReference<K, V> implements ValueReference<K, V> {
    ...
    public ListenableFuture<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
      ...
      try {
        ...
        return Futures.transform(newValue, (Function)new Object(this));
      } catch (Throwable t) {
    ...
    public LocalCache.ValueReference<K, V> copyFor(ReferenceQueue<V> queue, @Nullable V value, LocalCache.ReferenceEntry<K, V> entry) {
      return (LocalCache.ValueReference<K, V>)this;
    }
  }
  ...
  static final class WriteQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
    ...
    public Iterator<LocalCache.ReferenceEntry<K, V>> iterator() {
      return (Iterator<LocalCache.ReferenceEntry<K, V>>)new Object(this, peek());
    }
  }
  ...
  public Set<K> keySet() {
    ...
    return (ks != null) ? ks : (this.keySet = (Set<K>)new KeySet(this, (ConcurrentMap)this));
  }
  
  public Collection<V> values() {
    Collection<V> vs = this.values;
    return (vs != null) ? vs : (this.values = (Collection<V>)new Values(this, (ConcurrentMap)this));
  }
  
  @GwtIncompatible("Not supported.")
  public Set<Map.Entry<K, V>> entrySet() {
    Set<Map.Entry<K, V>> es = this.entrySet;
    return (es != null) ? es : (this.entrySet = (Set<Map.Entry<K, V>>)new EntrySet(this, (ConcurrentMap)this));
  }
  ...
  static class LocalManualCache<K, V> implements Cache<K, V>, Serializable {
    ...
    public V get(K key, Callable<? extends V> valueLoader) throws ExecutionException {
      ...
      return this.localCache.get(key, (CacheLoader<? super K, V>)new Object(this, valueLoader));
    }
