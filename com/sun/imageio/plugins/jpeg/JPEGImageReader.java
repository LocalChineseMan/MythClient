...
public class JPEGImageReader extends ImageReader {
  ...
  private void checkTablesOnly() throws IOException {
    ...
    if (this.seekForwardOnly) {
      Long long_ = this.imagePositions.get(this.imagePositions.size() - 1);
      this.iis.flushBefore(long_.longValue());
    } 
  ...
  private int getNumImagesOnThread(boolean paramBoolean) throws IOException {
    ...
    if (paramBoolean == true) {
      ...
      while (!bool) {
        ...
        int i = (jPEGBuffer.buf[jPEGBuffer.bufPtr++] & 0xFF) << 8 | jPEGBuffer.buf[jPEGBuffer.bufPtr++] & 0xFF;
        jPEGBuffer.bufAvail -= 2;
        i -= 2;
        jPEGBuffer.skipData(i);
      } 
      this.iis.reset();
      return this.numImages;
    } 
  ...
  private void gotoImage(int paramInt) throws IOException {
    if (this.iis == null)
      throw new IllegalStateException("Input not set"); 
    ...
    if (paramInt < this.imagePositions.size()) {
      ...
      Long long_ = this.imagePositions.get(this.imagePositions.size() - 1);
      this.iis.seek(long_.longValue());
      skipImage();
      int i = this.imagePositions.size();
      for (; i <= paramInt; i++) {
        if (!hasNextImage())
          throw new IndexOutOfBoundsException(); 
        long_ = new Long(this.iis.getStreamPosition());
        this.imagePositions.add(long_);
        if (this.seekForwardOnly)
          this.iis.flushBefore(long_.longValue()); 
        if (i < paramInt)
          skipImage(); 
  ...
  private void skipImage() throws IOException {
    ...
    boolean bool = false;
    int i = this.iis.read();
    for (; i != -1; i = this.iis.read()) {
      if (bool == true && i == 217)
        return; 
      bool = (i == 255) ? true : false;
    } 
    throw new IndexOutOfBoundsException();
  }
  ...
  private void pushBack(int paramInt) throws IOException {
    if (this.debug)
  ...
  private Iterator getImageTypesOnThread(int paramInt) throws IOException {
    ...
    ArrayList<ImageTypeProducer> arrayList = new ArrayList(1);
  ...
  private Raster readInternal(int paramInt, ImageReadParam paramImageReadParam, boolean paramBoolean) throws IOException {
    ...
    this.raster = Raster.createInterleavedRaster(this.buffer, this.destROI.width, 1, n, j, arrayOfInt2, (Point)null);
    if (paramBoolean) {
      this.target = Raster.createInterleavedRaster(0, this.destROI.width, this.destROI.height, n, j, arrayOfInt2, (Point)null);
    } else {
    ...
    for (b = 0; b < arrayOfInt3.length; b++) {
      if (arrayOfInt3[b] <= 0 || arrayOfInt3[b] > 8)
        throw new IIOException("Illegal band size: should be 0 < size <= 8"); 
    } 
    b = (this.updateListeners != null || this.progressListeners != null) ? 1 : 0;
    initProgressData();
    if (paramInt == this.imageMetadataIndex) {
      this.knownPassCount = 0;
      Iterator<MarkerSegment> iterator = this.imageMetadata.markerSequence.iterator();
    ...
    if (this.debug) {
      ...
      for (b1 = 0; b1 < arrayOfInt1.length; b1++)
        System.out.print(" " + arrayOfInt1[b1]); 
      System.out.println();
      System.out.println("destination bands is " + this.destinationBands);
      if (this.destinationBands != null) {
        for (b1 = 0; b1 < this.destinationBands.length; b1++)
          System.out.print(" " + this.destinationBands[b1]); 
        System.out.println();
      } 
      ...
      System.out.println("callbackUpdates is " + b);
    } 
    processImageStarted(this.currentImage);
    boolean bool = false;
    bool = readImage(this.structPointer, this.buffer.getData(), j, arrayOfInt1, arrayOfInt3, rectangle.x, rectangle.y, rectangle.width, rectangle.height, k, m, this.abbrevQTables, this.abbrevDCHuffmanTables, this.abbrevACHuffmanTables, this.minProgressivePass, this.maxProgressivePass, b);
    if (bool) {
      processReadAborted();
    } else {
  ...
  public Raster readRaster(int paramInt, ImageReadParam paramImageReadParam) throws IOException {
    ...
    try {
      ...
      raster = readInternal(paramInt, paramImageReadParam, true);
      if (point != null)
  ...
  public int getNumThumbnails(int paramInt) throws IOException {
    ...
    try {
      ...
      JFIFMarkerSegment jFIFMarkerSegment = (JFIFMarkerSegment)this.imageMetadata.findMarkerSegment((Class)JFIFMarkerSegment.class, true);
      int i = 0;
  ...
  private static class JPEGReaderDisposerRecord implements DisposerRecord {
    ...
    public JPEGReaderDisposerRecord(long param1Long) {
      this.pData = param1Long;
    }
