...
class ZoneNode extends NameNode {
  ...
  synchronized NameNode getContents() {
    return (this.contentsRef != null) ? this.contentsRef
      .get() : null;
  }
  ...
  NameNode populate(DnsName paramDnsName, ResourceRecords paramResourceRecords) {
    ...
    for (byte b = 0; b < paramResourceRecords.answer.size(); b++) {
      ResourceRecord resourceRecord1 = paramResourceRecords.answer.elementAt(b);
    ...
    ResourceRecord resourceRecord = paramResourceRecords.answer.firstElement();
    synchronized (this) {
  ...
  int compareSerialNumberTo(ResourceRecord paramResourceRecord) {
    return ResourceRecord.compareSerialNumbers(this.serialNumber, 
        getSerialNumber(paramResourceRecord));
  }
