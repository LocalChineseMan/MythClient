...
public class ResourceRecord {
  ...
  static final String[] rrTypeNames = new String[] { 
  ...
  ResourceRecord(byte[] paramArrayOfbyte, int paramInt1, int paramInt2, boolean paramBoolean1, boolean paramBoolean2) throws InvalidNameException {
  ...
  public String toString() {
    ...
    if (!this.qSection)
      str = str + " " + this.ttl + " " + ((this.rdata != null) ? (String)this.rdata : "[n/a]"); 
    return str;
  }
  ...
  private static int nameToValue(String paramString, String[] paramArrayOfString) {
    ...
    for (byte b = 1; b < paramArrayOfString.length; b++) {
      if (paramArrayOfString[b] != null && paramString
        .equalsIgnoreCase(paramArrayOfString[b]))
        return b; 
    } 
    return -1;
  }
  ...
  private void decode(boolean paramBoolean) throws InvalidNameException {
    int i = this.offset;
    ...
    if (!this.qSection) {
      ...
      this
        ...
        .rdata = (paramBoolean || this.rrtype == 6) ? decodeRdata(i) : null;
  ...
  private int getUByte(int paramInt) {
    return this.msg[paramInt] & 0xFF;
  }
  
  private int getUShort(int paramInt) {
    return (this.msg[paramInt] & 0xFF) << 8 | this.msg[paramInt + 1] & 0xFF;
  }
  ...
  private long getUInt(int paramInt) {
    return getInt(paramInt) & 0xFFFFFFFFL;
  }
  
  private DnsName decodeName(int paramInt) throws InvalidNameException {
    DnsName dnsName = new DnsName();
    decodeName(paramInt, dnsName);
    return dnsName;
  }
  
  private int decodeName(int paramInt, DnsName paramDnsName) throws InvalidNameException {
    if (this.msg[paramInt] == 0) {
      paramDnsName.add(0, "");
      return paramInt + 1;
    } 
    if ((this.msg[paramInt] & 0xC0) != 0) {
      decodeName(getUShort(paramInt) & 0x3FFF, paramDnsName);
      return paramInt + 2;
    } 
    byte b = this.msg[paramInt++];
    try {
      paramDnsName.add(0, new String(this.msg, paramInt, b, "ISO-8859-1"));
    } catch (UnsupportedEncodingException unsupportedEncodingException) {}
    return decodeName(paramInt + b, paramDnsName);
  }
  
  private Object decodeRdata(int paramInt) throws InvalidNameException {
    if (this.rrclass == 1)
      switch (this.rrtype) {
        case 1:
  ...
  private String decodeMx(int paramInt) throws InvalidNameException {
  ...
  private String decodeSoa(int paramInt) throws InvalidNameException {
  ...
  private String decodeSrv(int paramInt) throws InvalidNameException {
    int i = getUShort(paramInt);
    paramInt += 2;
  ...
  private String decodeNaptr(int paramInt) throws InvalidNameException {
    int i = getUShort(paramInt);
    paramInt += 2;
    int j = getUShort(paramInt);
    paramInt += 2;
  ...
  private String decodeA(int paramInt) {
    return (this.msg[paramInt] & 0xFF) + "." + (this.msg[paramInt + 1] & 0xFF) + "." + (this.msg[paramInt + 2] & 0xFF) + "." + (this.msg[paramInt + 3] & 0xFF);
  }
