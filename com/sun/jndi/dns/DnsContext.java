...
public class DnsContext extends ComponentDirContext {
  ...
  public DnsContext(String paramString, String[] paramArrayOfString, Hashtable<?, ?> paramHashtable) throws NamingException {
    ...
    this.environment = (Hashtable<Object, Object>)paramHashtable.clone();
  ...
  public Object addToEnvironment(String paramString, Object paramObject) throws NamingException {
    ...
    if (this.environment.get(paramString) != paramObject) {
      this.environment = (Hashtable<Object, Object>)this.environment.clone();
      this.envShared = false;
      return this.environment.put(paramString, paramObject);
    } 
    return paramObject;
  ...
  void setProviderUrl(String paramString) {
    this.environment.put("java.naming.provider.url", paramString);
  }
  ...
  private void initFromEnvironment() throws InvalidAttributeIdentifierException {
    ...
    this.authoritative = "true".equalsIgnoreCase((String)this.environment
        .get("java.naming.authoritative"));
  ...
  private CT getLookupCT(String paramString) throws InvalidAttributeIdentifierException {
    return (paramString == null) ? new CT(1, 16) : 
      
      fromAttrId(paramString);
  }
  ...
  public Name composeName(Name paramName1, Name paramName2) throws NamingException {
    ...
    Name name2 = (paramName2 instanceof CompositeName) ? paramName2 : (new CompositeName()).add(paramName2.toString());
    Name name3 = (paramName1 instanceof CompositeName) ? paramName1 : (new CompositeName()).add(paramName1.toString());
    int i = name2.size() - 1;
    if (name3.isEmpty() || name3.get(0).equals("") || name2
      .isEmpty() || name2.get(i).equals(""))
      return super.composeName(name3, name2); 
    Name name1 = (paramName2 == name2) ? (CompositeName)name2.clone() : name2;
    name1.addAll(name3);
    if (this.parentIsDns) {
      DnsName dnsName = (paramName2 instanceof DnsName) ? (DnsName)paramName2.clone() : new DnsName(name2.get(i));
      dnsName.addAll((paramName1 instanceof DnsName) ? paramName1 : new DnsName(name3
            
            .get(0)));
      name1.remove(i + 1);
      name1.remove(i);
      name1.add(i, dnsName.toString());
    } 
    return name1;
  }
  ...
  private static Attributes rrsToAttrs(ResourceRecords paramResourceRecords, CT[] paramArrayOfCT) {
    ...
    for (byte b = 0; b < paramResourceRecords.answer.size(); b++) {
      ResourceRecord resourceRecord = paramResourceRecords.answer.elementAt(b);
  ...
  private static boolean classAndTypeMatch(int paramInt1, int paramInt2, CT[] paramArrayOfCT) {
    ...
    for (byte b = 0; b < paramArrayOfCT.length; b++) {
      ...
      boolean bool1 = (cT.rrclass == 255 || cT.rrclass == paramInt1) ? true : false;
      boolean bool2 = (cT.rrtype == 255 || cT.rrtype == paramInt2) ? true : false;
      if (bool1 && bool2)
        return true; 
    } 
    return false;
  ...
  private NameNode getNameNode(DnsName paramDnsName) throws NamingException {
    ...
    dprint("Deepest related zone in zone tree: " + ((zoneNode != null) ? zoneNode
        .getLabel() : "[none]"));
    if (zoneNode != null) {
  ...
  private boolean isZoneCurrent(ZoneNode paramZoneNode, DnsName paramDnsName) throws NamingException {
    ...
    synchronized (paramZoneNode) {
      ...
      return (paramZoneNode.isPopulated() && paramZoneNode
        .compareSerialNumberTo(resourceRecord) >= 0);
    } 
  }
