...
abstract class MBeanIntrospector<M> {
  ...
  final PerInterface<M> getPerInterface(Class<?> paramClass) throws NotCompliantMBeanException {
    ...
    synchronized (perInterfaceMap) {
      WeakReference<PerInterface<M>> weakReference = perInterfaceMap.get(paramClass);
      PerInterface<M> perInterface = (weakReference == null) ? null : weakReference.get();
      if (perInterface == null)
        try {
          MBeanAnalyzer<M> mBeanAnalyzer = getAnalyzer(paramClass);
  ...
  final boolean consistent(M paramM1, M paramM2) {
    return (paramM1 == null || paramM2 == null || 
      getGenericReturnType(paramM1).equals(getGenericParameterTypes(paramM2)[0]));
  }
  ...
  private static void unwrapInvocationTargetException(InvocationTargetException paramInvocationTargetException) throws MBeanException {
    ...
    throw new MBeanException((Exception)throwable, (throwable == null) ? null : throwable
        .toString());
  }
  ...
  private class MBeanInfoMaker implements MBeanAnalyzer.MBeanVisitor<M> {
    ...
    MBeanInfo makeMBeanInfo(Class<?> param1Class, String param1String) {
      MBeanAttributeInfo[] arrayOfMBeanAttributeInfo = this.attrs.<MBeanAttributeInfo>toArray(new MBeanAttributeInfo[0]);
      MBeanOperationInfo[] arrayOfMBeanOperationInfo = this.ops.<MBeanOperationInfo>toArray(new MBeanOperationInfo[0]);
  ...
  final MBeanInfo getMBeanInfo(Object paramObject, PerInterface<M> paramPerInterface) {
    ...
    if (arrayOfMBeanNotificationInfo == null || arrayOfMBeanNotificationInfo.length == 0)
      return mBeanInfo; 
    return new MBeanInfo(mBeanInfo.getClassName(), mBeanInfo
        .getDescription(), mBeanInfo
        .getAttributes(), mBeanInfo
        .getConstructors(), mBeanInfo
        .getOperations(), arrayOfMBeanNotificationInfo, mBeanInfo
  ...
  final MBeanInfo getClassMBeanInfo(Class<?> paramClass, PerInterface<M> paramPerInterface) {
    ...
    synchronized (mBeanInfoMap) {
      WeakHashMap<Class<?>, MBeanInfo> weakHashMap = mBeanInfoMap.get(paramClass);
      if (weakHashMap == null) {
        weakHashMap = new WeakHashMap<>();
        mBeanInfoMap.put(paramClass, weakHashMap);
      } 
      Class<?> clazz = paramPerInterface.getMBeanInterface();
      MBeanInfo mBeanInfo = weakHashMap.get(clazz);
      if (mBeanInfo == null) {
        MBeanInfo mBeanInfo1 = paramPerInterface.getMBeanInfo();
        ImmutableDescriptor immutableDescriptor = ImmutableDescriptor.union(new Descriptor[] { mBeanInfo1.getDescriptor(), 
              getMBeanDescriptor(paramClass) });
