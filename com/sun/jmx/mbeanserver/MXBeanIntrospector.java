...
class MXBeanIntrospector extends MBeanIntrospector<ConvertingMethod> {
  ...
  MBeanAttributeInfo getMBeanAttributeInfo(String paramString, ConvertingMethod paramConvertingMethod1, ConvertingMethod paramConvertingMethod2) {
    ...
    boolean bool1 = (paramConvertingMethod1 != null) ? true : false;
    boolean bool2 = (paramConvertingMethod2 != null) ? true : false;
    boolean bool3 = (bool1 && getName(paramConvertingMethod1).startsWith("is")) ? true : false;
    ...
    if (bool1)
      descriptor = ImmutableDescriptor.union(new Descriptor[] { descriptor, paramConvertingMethod1
            .getDescriptor() }); 
    if (bool2)
      descriptor = ImmutableDescriptor.union(new Descriptor[] { descriptor, paramConvertingMethod2
            .getDescriptor() }); 
  ...
  MBeanOperationInfo getMBeanOperationInfo(String paramString, ConvertingMethod paramConvertingMethod) {
    ...
    OpenType[] arrayOfOpenType = (OpenType[])paramConvertingMethod.getOpenParameterTypes();
    ...
    for (byte b = 0; b < arrayOfOpenType.length; b++) {
      ...
      descriptor1 = ImmutableDescriptor.union(new Descriptor[] { descriptor1, 
            Introspector.descriptorForAnnotations(arrayOfAnnotation[b]) });
      if (canUseOpenInfo(type1)) {
        mBeanParameterInfo = new OpenMBeanParameterInfoSupport(str1, str2, openType1, descriptor1);
      } else {
        bool1 = false;
        mBeanParameterInfo = new MBeanParameterInfo(str1, originalTypeString(type1), str2, descriptor1);
      } 
      arrayOfMBeanParameterInfo[b] = mBeanParameterInfo;
    } 
    Descriptor descriptor = typeDescriptor(openType, type);
    descriptor = ImmutableDescriptor.union(new Descriptor[] { descriptor, 
  ...
  private static Descriptor typeDescriptor(OpenType<?> paramOpenType, Type paramType) {
    return new ImmutableDescriptor(new String[] { "openType", "originalType" }, new Object[] { paramOpenType, 
          
          originalTypeString(paramType) });
  }
  
  private static boolean canUseOpenInfo(Type paramType) {
    if (paramType instanceof GenericArrayType)
      return canUseOpenInfo(((GenericArrayType)paramType)
          .getGenericComponentType()); 
    if (paramType instanceof Class && ((Class)paramType).isArray())
      return canUseOpenInfo(((Class)paramType)
          .getComponentType()); 
