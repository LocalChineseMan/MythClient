...
public class CallbackReference extends WeakReference<Callback> {
  ...
  private static Method getCallbackMethod(Class<?> cls) {
    ...
    Method[] methods = pmethods.<Method>toArray(new Method[pmethods.size()]);
    if (methods.length == 1)
  ...
  private class DefaultCallbackProxy implements CallbackProxy {
    ...
    public DefaultCallbackProxy(Method callbackMethod, TypeMapper mapper, String encoding) {
      ...
      for (int i = 0; i < this.fromNative.length; i++) {
        if (NativeMapped.class.isAssignableFrom(argTypes[i])) {
          this.fromNative[i] = (FromNativeConverter)new NativeMappedConverter(argTypes[i]);
        } else if (mapper != null) {
    ...
    private Object invokeCallback(Object[] args) {
      ...
      for (int i = 0; i < args.length; i++) {
        ...
        if (this.fromNative[i] != null) {
          CallbackParameterContext callbackParameterContext = new CallbackParameterContext(type, this.callbackMethod, args, i);
          callbackArgs[i] = this.fromNative[i].fromNative(arg, (FromNativeContext)callbackParameterContext);
        } else {
    ...
    private Object convertResult(Object value) {
      if (this.toNative != null)
        value = this.toNative.toNative(value, (ToNativeContext)new CallbackResultContext(this.callbackMethod)); 
      if (value == null)
        return null; 
