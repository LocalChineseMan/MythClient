...
public class ViaManagerImpl implements ViaManager {
  ...
  private final ConnectionManager connectionManager = (ConnectionManager)new ConnectionManagerImpl();
  ...
  public void onServerLoaded() {
    ...
    if (serverProtocolVersion < ProtocolVersion.v1_9.getVersion() && 
      ...
      Via.getPlatform().runRepeatingSync((Runnable)new ViaIdleThread(), 1L); 
    if (serverProtocolVersion < ProtocolVersion.v1_13.getVersion() && 
      Via.getConfig().get1_13TabCompleteDelay() > 0)
      Via.getPlatform().runRepeatingSync((Runnable)new TabCompleteThread(), 1L); 
  ...
  private void loadServerProtocol() {
    try {
      ...
      if (this.platform.isProxy()) {
        ...
        ServerProtocolVersionRange serverProtocolVersionRange = new ServerProtocolVersionRange(supportedVersions.firstInt(), supportedVersions.lastInt(), supportedVersions);
      } else {
        serverProtocolVersionSingleton = new ServerProtocolVersionSingleton(serverProtocolVersion.getVersion());
      } 
      this.protocolManager.setServerProtocol((ServerProtocolVersion)serverProtocolVersionSingleton);
    } catch (Exception e) {
  ...
  public ProtocolManager getProtocolManager() {
    return (ProtocolManager)this.protocolManager;
  }
