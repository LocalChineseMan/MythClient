...
public class MetadataRewriter1_15To1_14_4 extends EntityRewriter<Protocol1_15To1_14_4> {
  ...
  public void handleMetadata(int entityId, EntityType type, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
    ...
    if (type == null)
      return; 
    if (type.isOrHasParent((EntityType)Entity1_15Types.MINECART_ABSTRACT) && metadata
      .id() == 10) {
      int data = ((Integer)metadata.getValue()).intValue();
      metadata.setValue(Integer.valueOf(((Protocol1_15To1_14_4)this.protocol).getMappingData().getNewBlockStateId(data)));
    } 
    if (metadata.id() > 11 && type.isOrHasParent((EntityType)Entity1_15Types.LIVINGENTITY))
      metadata.setId(metadata.id() + 1); 
    if (type.isOrHasParent((EntityType)Entity1_15Types.WOLF))
      if (metadata.id() == 18) {
