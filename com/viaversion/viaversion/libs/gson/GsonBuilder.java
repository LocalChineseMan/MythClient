...
public final class GsonBuilder {
  ...
  private void addTypeAdaptersForDate(String datePattern, int dateStyle, int timeStyle, List<TypeAdapterFactory> factories) {
    DefaultDateTypeAdapter dateTypeAdapter, defaultDateTypeAdapter1, defaultDateTypeAdapter2;
    if (datePattern != null && !"".equals(datePattern.trim())) {
      dateTypeAdapter = new DefaultDateTypeAdapter(Date.class, datePattern);
      defaultDateTypeAdapter1 = new DefaultDateTypeAdapter(Timestamp.class, datePattern);
      defaultDateTypeAdapter2 = new DefaultDateTypeAdapter(Date.class, datePattern);
    } else if (dateStyle != 2 && timeStyle != 2) {
      dateTypeAdapter = new DefaultDateTypeAdapter(Date.class, dateStyle, timeStyle);
      defaultDateTypeAdapter1 = new DefaultDateTypeAdapter(Timestamp.class, dateStyle, timeStyle);
      defaultDateTypeAdapter2 = new DefaultDateTypeAdapter(Date.class, dateStyle, timeStyle);
    } else {
      return;
    } 
    factories.add(TypeAdapters.newFactory(Date.class, (TypeAdapter)dateTypeAdapter));
    factories.add(TypeAdapters.newFactory(Timestamp.class, (TypeAdapter)defaultDateTypeAdapter1));
    factories.add(TypeAdapters.newFactory(Date.class, (TypeAdapter)defaultDateTypeAdapter2));
  }
}
