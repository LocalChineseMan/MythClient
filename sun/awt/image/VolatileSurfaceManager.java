...
public abstract class VolatileSurfaceManager extends SurfaceManager implements DisplayChangedListener {
  ...
  protected VolatileSurfaceManager(SunVolatileImage paramSunVolatileImage, Object paramObject) {
    ...
    if (graphicsEnvironment instanceof SunGraphicsEnvironment)
      ((SunGraphicsEnvironment)graphicsEnvironment).addDisplayChangedListener((DisplayChangedListener)this); 
  }
  ...
  public void initialize() {
    ...
    if (this.sdCurrent == null && this.vImg
      .getForcedAccelSurfaceType() == 0)
  ...
  protected SurfaceData getBackupSurface() {
    if (this.sdBackup == null) {
      ...
      SunWritableRaster.stealTrackable(bufferedImage
          .getRaster()
  ...
  protected boolean isConfigValid(GraphicsConfiguration paramGraphicsConfiguration) {
    return (paramGraphicsConfiguration == null || paramGraphicsConfiguration
      .getDevice() == this.vImg.getGraphicsConfig().getDevice());
  }
  
  public ImageCapabilities getCapabilities(GraphicsConfiguration paramGraphicsConfiguration) {
    if (isConfigValid(paramGraphicsConfiguration))
      return isAccelerationEnabled() ? new AcceleratedImageCapabilities(this) : new ImageCapabilities(false); 
  ...
  private class VolatileSurfaceManager {}
}
