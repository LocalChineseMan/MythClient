...
public class IntegerInterleavedRaster extends IntegerComponentRaster {
  ...
  public IntegerInterleavedRaster(SampleModel paramSampleModel, Point paramPoint) {
    this(paramSampleModel, paramSampleModel
        .createDataBuffer(), new Rectangle(paramPoint.x, paramPoint.y, paramSampleModel
          ...
          .getHeight()), paramPoint, (IntegerInterleavedRaster)null);
  }
  ...
  private void setDataElements(int paramInt1, int paramInt2, int paramInt3, int paramInt4, Raster paramRaster) {
    ...
    if (paramRaster instanceof IntegerInterleavedRaster) {
      ...
      int k = integerInterleavedRaster.getScanlineStride();
      int m = integerInterleavedRaster.getDataOffset(0);
      int n = m;
      int i1 = this.dataOffsets[0] + (paramInt2 - this.minY) * this.scanlineStride + paramInt1 - this.minX;
      for (byte b1 = 0; b1 < paramInt4; b1++) {
        System.arraycopy(arrayOfInt, n, this.data, i1, paramInt3);
        n += k;
