...
public final class AWTAutoShutdown implements Runnable {
  ...
  private boolean isReadyToShutdown() {
    return (!this.toolkitThreadBusy && this.peerMap
      .isEmpty() && this.busyThreadSet
      .isEmpty());
  }
    ...
    if (paramPlatformLogger.isLoggable(PlatformLogger.Level.FINE))
      synchronized (this.activationLock) {
        synchronized (this.mainLock) {
          ...
          for (String str : this.peerMap.keySet())
            paramPlatformLogger.fine(str + "->" + this.peerMap.get(str)); 
        } 
      }  
  }
}
