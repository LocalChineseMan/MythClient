...
public final class SunGraphics2D extends Graphics2D implements ConstrainableGraphics, Cloneable, DestSurfaceProvider {
  ...
  Shape intersectRectShape(Rectangle2D paramRectangle2D, Shape paramShape, boolean paramBoolean1, boolean paramBoolean2) {
    if (paramShape instanceof Rectangle2D) {
      Rectangle2D rectangle2D2, rectangle2D1 = (Rectangle2D)paramShape;
      if (!paramBoolean1) {
        rectangle2D2 = paramRectangle2D;
      } else if (!paramBoolean2) {
        rectangle2D2 = rectangle2D1;
      } else {
        rectangle2D2 = new Rectangle2D.Float();
      } 
      double d1 = Math.max(paramRectangle2D.getX(), rectangle2D1.getX());
      double d2 = Math.min(paramRectangle2D.getX() + paramRectangle2D.getWidth(), rectangle2D1
          .getX() + rectangle2D1.getWidth());
      double d3 = Math.max(paramRectangle2D.getY(), rectangle2D1.getY());
      double d4 = Math.min(paramRectangle2D.getY() + paramRectangle2D.getHeight(), rectangle2D1
          .getY() + rectangle2D1.getHeight());
      if (d2 - d1 < 0.0D || d4 - d3 < 0.0D) {
        rectangle2D2.setFrameFromDiagonal(0.0D, 0.0D, 0.0D, 0.0D);
      } else {
        rectangle2D2.setFrameFromDiagonal(d1, d3, d2, d4);
      } 
      return rectangle2D2;
    } 
  ...
  private static final double[] IDENT_MATRIX = new double[] { 1.0D, 0.0D, 0.0D, 1.0D };
  ...
  private static double[][] textTxArr = new double[17][];
  
  private static AffineTransform[] textAtArr = new AffineTransform[17];
  ...
  public static boolean isRotated(double[] paramArrayOfdouble) {
    if (paramArrayOfdouble[0] == paramArrayOfdouble[3] && paramArrayOfdouble[1] == 0.0D && paramArrayOfdouble[2] == 0.0D && paramArrayOfdouble[0] > 0.0D)
      return false; 
    return true;
  }
  ...
  public void setFont(Font paramFont) {
    if (paramFont != null && paramFont != this.font) {
      if (this.textAntialiasHint == 3 && this.textpipe != invalidpipe && (this.transformState > 2 || paramFont
        
        .isTransformed() || this.fontInfo == null || ((this.fontInfo.aaHint == 2)) != 
        
        FontUtilities.getFont2D(paramFont)
        .useAAForPtSize(paramFont.getSize())))
        this.textpipe = invalidpipe; 
      this.font = paramFont;
  ...
  public void setComposite(Composite paramComposite) {
    ...
    if (paramComposite instanceof AlphaComposite) {
      ...
      if (compositeType == CompositeType.SrcOverNoEa) {
        if (this.paintState == 0 || (this.paintState > 1 && this.paint
  ...
  public void setPaint(Paint paramPaint) {
    ...
    if (clazz == GradientPaint.class) {
      this.paintState = 2;
    } else if (clazz == LinearGradientPaint.class) {
      this.paintState = 3;
    } else if (clazz == RadialGradientPaint.class) {
      this.paintState = 4;
    } else if (clazz == TexturePaint.class) {
      this.paintState = 5;
    } else {
      this.paintState = 6;
    } 
  ...
  private void validateBasicStroke(BasicStroke paramBasicStroke) {
    boolean bool = (this.antialiasHint == 2) ? true : false;
    if (this.transformState < 3) {
      if (bool) {
  ...
  public void setRenderingHint(RenderingHints.Key paramKey, Object paramObject) {
    ...
    if (paramKey instanceof SunHints.Key) {
      ...
      switch (key.getIndex()) {
        case 0:
          bool1 = (this.renderHint != i) ? true : false;
          if (bool1) {
            this.renderHint = i;
          ...
          break;
        case 1:
          bool1 = (this.antialiasHint != i) ? true : false;
          this.antialiasHint = i;
          if (bool1) {
            bool2 = (this.textAntialiasHint == 0) ? true : false;
            if (this.strokeState != 3)
              validateBasicStroke((BasicStroke)this.stroke); 
          } 
          break;
        case 2:
          bool1 = (this.textAntialiasHint != i) ? true : false;
          bool2 = bool1;
          this.textAntialiasHint = i;
          break;
        case 3:
          bool1 = (this.fractionalMetricsHint != i) ? true : false;
          bool2 = bool1;
        ...
        case 5:
          ...
          bool1 = (this.interpolationType != i) ? true : false;
          this.interpolationType = i;
          break;
        case 8:
          bool1 = (this.strokeHint != i) ? true : false;
          this.strokeHint = i;
          break;
        case 9:
          bool1 = (this.resolutionVariantHint != i) ? true : false;
  ...
  public void setRenderingHints(Map<?, ?> paramMap) {
    ...
    while (iterator.hasNext()) {
      SunHints.Key key = (SunHints.Key)iterator.next();
      if (key == SunHints.KEY_RENDERING || key == SunHints.KEY_ANTIALIASING || key == SunHints.KEY_TEXT_ANTIALIASING || key == SunHints.KEY_FRACTIONALMETRICS || key == SunHints.KEY_TEXT_ANTIALIAS_LCD_CONTRAST || key == SunHints.KEY_STROKE_CONTROL || key == SunHints.KEY_INTERPOLATION) {
        setRenderingHint(key, paramMap.get(key));
        continue;
      } 
      bool = true;
    } 
  ...
  RenderingHints makeHints(Map<RenderingHints.Key, ?> paramMap) {
    ...
    renderingHints.put(SunHints.KEY_RENDERING, 
        SunHints.Value.get(0, this.renderHint));
    renderingHints.put(SunHints.KEY_ANTIALIASING, 
        SunHints.Value.get(1, this.antialiasHint));
    renderingHints.put(SunHints.KEY_TEXT_ANTIALIASING, 
        SunHints.Value.get(2, this.textAntialiasHint));
    renderingHints.put(SunHints.KEY_FRACTIONALMETRICS, 
        SunHints.Value.get(3, this.fractionalMetricsHint));
    renderingHints.put(SunHints.KEY_TEXT_ANTIALIAS_LCD_CONTRAST, 
        Integer.valueOf(this.lcdTextContrast));
    switch (this.interpolationHint) {
      case 0:
    ...
    renderingHints.put(SunHints.KEY_STROKE_CONTROL, 
        SunHints.Value.get(8, this.strokeHint));
  ...
  public boolean hitClip(int paramInt1, int paramInt2, int paramInt3, int paramInt4) {
    ...
    if (this.transformState > 1) {
      ...
      paramInt1 = (int)Math.floor(Math.min(Math.min(arrayOfDouble[0], arrayOfDouble[2]), 
            Math.min(arrayOfDouble[4], arrayOfDouble[6])));
      paramInt2 = (int)Math.floor(Math.min(Math.min(arrayOfDouble[1], arrayOfDouble[3]), 
            Math.min(arrayOfDouble[5], arrayOfDouble[7])));
      paramInt3 = (int)Math.ceil(Math.max(Math.max(arrayOfDouble[0], arrayOfDouble[2]), 
            Math.max(arrayOfDouble[4], arrayOfDouble[6])));
      paramInt4 = (int)Math.ceil(Math.max(Math.max(arrayOfDouble[1], arrayOfDouble[3]), 
            Math.max(arrayOfDouble[5], arrayOfDouble[7])));
    } else {
  ...
  protected void validateCompClip() {
    ...
    if (this.usrClip == null) {
      ...
      ShapeSpanIterator shapeSpanIterator = LoopPipe.getFillSSI(this);
      try {
        ...
        this
          .clipState = region.isRectangular() ? 1 : 2;
      } finally {
  ...
  protected static Shape transformShape(int paramInt1, int paramInt2, Shape paramShape) {
    ...
    if (paramShape instanceof Rectangle2D) {
      ...
      return new Rectangle2D.Double(rectangle2D.getX() + paramInt1, rectangle2D
          .getY() + paramInt2, rectangle2D
  ...
  protected static Shape transformShape(AffineTransform paramAffineTransform, Shape paramShape) {
    if (paramShape == null)
      return null; 
    if (paramShape instanceof Rectangle2D && (paramAffineTransform
      .getType() & 0x30) == 0) {
      Rectangle2D rectangle2D = (Rectangle2D)paramShape;
  ...
  private static void fixRectangleOrientation(double[] paramArrayOfdouble, Rectangle2D paramRectangle2D) {
    if (((paramRectangle2D.getWidth() > 0.0D) ? true : false) != ((paramArrayOfdouble[2] - paramArrayOfdouble[0] > 0.0D) ? true : false)) {
      double d = paramArrayOfdouble[0];
      paramArrayOfdouble[0] = paramArrayOfdouble[2];
      paramArrayOfdouble[2] = d;
    } 
    if (((paramRectangle2D.getHeight() > 0.0D) ? true : false) != ((paramArrayOfdouble[3] - paramArrayOfdouble[1] > 0.0D) ? true : false)) {
  ...
  private void doCopyArea(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int paramInt6) {
    ...
    if (surfaceData.copyArea(this, paramInt1, paramInt2, paramInt3, paramInt4, paramInt5, paramInt6))
      return; 
    if (this.transformState > 3)
    ...
    if (this.lastCAcomp != composite) {
      ...
      if (CompositeType.SrcOverNoEa.equals(compositeType) && surfaceData
        .getTransparency() == 1)
    ...
    if (paramInt6 == 0 && paramInt5 > 0 && paramInt5 < paramInt3) {
      while (paramInt3 > 0) {
        ...
        blit.Blit(surfaceData, surfaceData, composite, region, j, paramInt2, j + paramInt5, paramInt2 + paramInt6, i, paramInt4);
      } 
      return;
    } 
    if (paramInt6 > 0 && paramInt6 < paramInt4 && paramInt5 > -paramInt3 && paramInt5 < paramInt3) {
      while (paramInt4 > 0) {
        ...
        blit.Blit(surfaceData, surfaceData, composite, region, paramInt1, j, paramInt1 + paramInt5, j + paramInt6, paramInt3, i);
      } 
      return;
    } 
    blit.Blit(surfaceData, surfaceData, composite, region, paramInt1, paramInt2, paramInt1 + paramInt5, paramInt2 + paramInt6, paramInt3, paramInt4);
  }
  ...
  private static Rectangle getImageRegion(RenderedImage paramRenderedImage, Region paramRegion, AffineTransform paramAffineTransform1, AffineTransform paramAffineTransform2, int paramInt1, int paramInt2) {
    ...
    try {
      ...
      double d2 = arrayOfDouble[0], d1 = d2;
      double d4 = arrayOfDouble[1], d3 = d4;
      int i;
      for (i = 2; i < 8; ) {
  ...
  public void drawRenderedImage(RenderedImage paramRenderedImage, AffineTransform paramAffineTransform) {
    ...
    boolean bool1 = (this.transformState <= 1 && isIntegerTranslation(paramAffineTransform)) ? true : false;
    boolean bool2 = bool1 ? false : true;
    try {
      region = getCompClip();
    } catch (InvalidPipeException invalidPipeException) {
      return;
    } 
    Rectangle rectangle = getImageRegion(paramRenderedImage, region, this.transform, paramAffineTransform, bool2, bool2);
    if (rectangle.width <= 0 || rectangle.height <= 0)
      return; 
    if (bool1) {
      drawTranslatedRenderedImage(paramRenderedImage, rectangle, 
          (int)paramAffineTransform.getTranslateX(), 
          (int)paramAffineTransform.getTranslateY());
      return;
    } 
    Raster raster = paramRenderedImage.getData(rectangle);
    WritableRaster writableRaster = Raster.createWritableRaster(raster.getSampleModel(), raster
        .getDataBuffer(), null);
    ...
    if (n != 0 || i1 != 0 || k != writableRaster.getWidth() || m != writableRaster
      .getHeight())
      writableRaster = writableRaster.createWritableChild(n, i1, k, m, 0, 0, (int[])null); 
    ...
    for (int i4 = i1; i4 <= i3; i4++) {
      for (int i5 = n; i5 <= i2; i5++) {
        ...
        if (raster instanceof WritableRaster) {
          ...
          writableRaster = Raster.createWritableRaster(raster.getSampleModel(), raster
              .getDataBuffer(), null);
        } 
        writableRaster = writableRaster.createWritableChild(rectangle.x, rectangle.y, rectangle.width, rectangle.height, 0, 0, (int[])null);
        BufferedImage bufferedImage = new BufferedImage(colorModel, writableRaster, colorModel.isAlphaPremultiplied(), null);
  ...
  public void drawChars(char[] paramArrayOfchar, int paramInt1, int paramInt2, int paramInt3, int paramInt4) {
    ...
    if (this.font.hasLayoutAttributes()) {
      if (paramArrayOfchar.length == 0)
        return; 
      (new TextLayout(new String(paramArrayOfchar, paramInt1, paramInt2), this.font, 
          getFontRenderContext())).draw(this, paramInt3, paramInt4);
      return;
    } 
    try {
  ...
  public void drawBytes(byte[] paramArrayOfbyte, int paramInt1, int paramInt2, int paramInt3, int paramInt4) {
    ...
    if (this.font.hasLayoutAttributes()) {
      if (paramArrayOfbyte.length == 0)
        return; 
      (new TextLayout(new String(arrayOfChar), this.font, 
          getFontRenderContext())).draw(this, paramInt3, paramInt4);
      return;
    } 
    try {
  ...
  private boolean isHiDPIImage(Image paramImage) {
    return (SurfaceManager.getImageScale(paramImage) != 1 || (this.resolutionVariantHint != 1 && paramImage instanceof MultiResolutionImage));
  }
  ...
  private Image getResolutionVariant(MultiResolutionImage paramMultiResolutionImage, int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int paramInt6, int paramInt7, int paramInt8, int paramInt9, int paramInt10) {
    ...
    if ((k & 0xFFFFFFBE) == 0) {
      ...
      d1 = m * Math.hypot(this.transform
          .getScaleX(), this.transform.getShearY());
      d2 = n * Math.hypot(this.transform
          .getShearX(), this.transform.getScaleY());
    } 
    ...
    if (image instanceof ToolkitImage && ((ToolkitImage)image)
      .hasError())
      return null; 
    return image;
  }
  ...
  public FontRenderContext getFontRenderContext() {
    if (this.cachedFRC == null) {
      ...
      if (this.transformState >= 3)
        if (this.transform.getTranslateX() == 0.0D && this.transform
          .getTranslateY() == 0.0D) {
      ...
      this
        ...
        .cachedFRC = new FontRenderContext(affineTransform, SunHints.Value.get(2, i), SunHints.Value.get(3, this.fractionalMetricsHint));
    } 
