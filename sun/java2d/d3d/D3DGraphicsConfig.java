...
public class D3DGraphicsConfig extends Win32GraphicsConfig implements AccelGraphicsConfig {
  ...
  public SurfaceData createManagedSurface(int paramInt1, int paramInt2, int paramInt3) {
    return D3DSurfaceData.createData(this, paramInt1, paramInt2, 
        getColorModel(paramInt3), null, 3);
  }
  ...
  public void flip(WComponentPeer paramWComponentPeer, Component paramComponent, VolatileImage paramVolatileImage, int paramInt1, int paramInt2, int paramInt3, int paramInt4, BufferCapabilities.FlipContents paramFlipContents) {
    ...
    if (surfaceData instanceof D3DSurfaceData) {
      ...
      D3DSurfaceData.swapBuffers(d3DSurfaceData, paramInt1, paramInt2, paramInt3, paramInt4);
    } else {
      Graphics graphics = paramWComponentPeer.getGraphics();
      try {
    ...
    if (paramFlipContents == BufferCapabilities.FlipContents.BACKGROUND) {
      Graphics graphics = paramVolatileImage.getGraphics();
      try {
        graphics.setColor(paramComponent.getBackground());
        graphics.fillRect(0, 0, paramVolatileImage
            .getWidth(), paramVolatileImage
            .getHeight());
      } finally {
        graphics.dispose();
      } 
  ...
  public VolatileImage createCompatibleVolatileImage(int paramInt1, int paramInt2, int paramInt3, int paramInt4) {
    ...
    if (paramInt4 == 5) {
      ...
      !bool && !this.device.isCapPresent(2)) {
      return null;
    } 
    AccelTypedVolatileImage accelTypedVolatileImage = new AccelTypedVolatileImage((GraphicsConfiguration)this, paramInt1, paramInt2, paramInt3, paramInt4);
    Surface surface = accelTypedVolatileImage.getDestSurface();
    if (!(surface instanceof AccelSurface) || ((AccelSurface)surface)
      .getType() != paramInt4) {
  ...
  private static class D3DGraphicsConfig {}
}
