...
public class PixelToParallelogramConverter extends PixelToShapeConverter implements ShapeDrawPipe {
  ...
  public void drawRect(SunGraphics2D paramSunGraphics2D, int paramInt1, int paramInt2, int paramInt3, int paramInt4) {
    if (paramInt3 >= 0 && paramInt4 >= 0) {
      if (paramSunGraphics2D.strokeState < 3) {
        ...
        if (paramInt3 > 0 && paramInt4 > 0) {
          if (basicStroke.getLineJoin() == 0 && basicStroke
            .getDashArray() == null) {
  ...
  public void draw(SunGraphics2D paramSunGraphics2D, Shape paramShape) {
    if (paramSunGraphics2D.strokeState < 3) {
      ...
      if (paramShape instanceof Rectangle2D) {
        ...
        if (drawGeneralLine(paramSunGraphics2D, line2D
            .getX1(), line2D.getY1(), line2D
            .getX2(), line2D.getY2()))
          return; 
      } 
    } 
  ...
  static double len(double paramDouble1, double paramDouble2) {
    return (paramDouble1 == 0.0D) ? Math.abs(paramDouble2) : ((paramDouble2 == 0.0D) ? 
      Math.abs(paramDouble1) : 
      Math.sqrt(paramDouble1 * paramDouble1 + paramDouble2 * paramDouble2));
  }
