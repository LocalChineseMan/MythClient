...
public class X509Factory extends CertificateFactorySpi {
  ...
  public Certificate engineGenerateCertificate(InputStream paramInputStream) throws CertificateException {
    ...
    try {
      ...
      if (arrayOfByte != null) {
        X509CertImpl x509CertImpl = getFromCache(certCache, arrayOfByte);
      ...
      throw new CertificateException("Could not parse certificate: " + iOException
  ...
  public static synchronized X509CRLImpl intern(X509CRL paramX509CRL) throws CRLException {
    ...
    X509CRLImpl x509CRLImpl = getFromCache(crlCache, arrayOfByte);
    if (x509CRLImpl != null)
      return x509CRLImpl; 
    if (bool) {
  ...
  private static synchronized <K, V> V getFromCache(Cache<K, V> paramCache, byte[] paramArrayOfbyte) {
    ...
    return paramCache.get(equalByteArray);
  }
  ...
  private Collection<? extends Certificate> parseX509orPKCS7Cert(InputStream paramInputStream) throws CertificateException, IOException {
    ...
    try {
      ...
      if (arrayOfX509Certificate != null)
        return Arrays.asList((Certificate[])arrayOfX509Certificate); 
      return new ArrayList<>(0);
  ...
  private Collection<? extends CRL> parseX509orPKCS7CRL(InputStream paramInputStream) throws CRLException, IOException {
    PushbackInputStream pushbackInputStream = new PushbackInputStream(paramInputStream);
    ArrayList<X509CRLImpl> arrayList = new ArrayList();
    int i = pushbackInputStream.read();
    if (i == -1)
      return new ArrayList<>(0); 
    pushbackInputStream.unread(i);
    ...
    try {
      ...
      if (arrayOfX509CRL != null)
        return Arrays.asList((CRL[])arrayOfX509CRL); 
      return new ArrayList<>(0);
    } catch (ParsingException parsingException) {
      while (arrayOfByte != null) {
  ...
  private static byte[] readOneBlock(InputStream paramInputStream) throws IOException {
    ...
    return Base64.getMimeDecoder().decode(new String(arrayOfChar, 0, b1));
  }
  
  private static void checkHeaderFooter(String paramString1, String paramString2) throws IOException {
    if (paramString1.length() < 16 || !paramString1.startsWith("-----BEGIN ") || 
      !paramString1.endsWith("-----"))
      throw new IOException("Illegal header: " + paramString1); 
    if (paramString2.length() < 14 || !paramString2.startsWith("-----END ") || 
