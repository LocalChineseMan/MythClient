...
final class PolicyNodeImpl implements PolicyNode {
  ...
  PolicyNodeImpl(PolicyNodeImpl paramPolicyNodeImpl, String paramString, Set<PolicyQualifierInfo> paramSet, boolean paramBoolean1, Set<String> paramSet1, boolean paramBoolean2) {
    ...
    if (paramSet != null) {
      this.mQualifierSet = new HashSet<>(paramSet);
    } else {
      this.mQualifierSet = new HashSet<>();
    } 
    this.mCriticalityIndicator = paramBoolean1;
    if (paramSet1 != null) {
      this.mExpectedPolicySet = new HashSet<>(paramSet1);
    } else {
  ...
  public Iterator<PolicyNodeImpl> getChildren() {
    return Collections.<PolicyNodeImpl>unmodifiableSet(this.mChildren).iterator();
  }
