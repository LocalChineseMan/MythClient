...
class AdaptableX509CertSelector extends X509CertSelector {
  ...
  void setSkiAndSerialNumber(AuthorityKeyIdentifierExtension paramAuthorityKeyIdentifierExtension) throws IOException {
    ...
    if (paramAuthorityKeyIdentifierExtension != null) {
      KeyIdentifier keyIdentifier = (KeyIdentifier)paramAuthorityKeyIdentifierExtension.get("key_id");
      if (keyIdentifier != null) {
        DerOutputStream derOutputStream = new DerOutputStream();
        derOutputStream.putOctetString(keyIdentifier.getIdentifier());
        this.ski = derOutputStream.toByteArray();
      } 
  ...
  public boolean match(Certificate paramCertificate) {
    ...
    if (this.serial != null && i > 2 && 
      !this.serial.equals(x509Certificate.getSerialNumber()))
      return false; 
    if (i < 3) {
  ...
  private boolean matchSubjectKeyID(X509Certificate paramX509Certificate) {
    ...
    try {
      ...
      if (arrayOfByte1 == null) {
        if (debug != null)
          debug.println("AdaptableX509CertSelector.match: no subject key ID extension"); 
        return true;
      } 
      DerInputStream derInputStream = new DerInputStream(arrayOfByte1);
      byte[] arrayOfByte2 = derInputStream.getOctetString();
      if (arrayOfByte2 == null || 
        !Arrays.equals(this.ski, arrayOfByte2)) {
        if (debug != null)
          debug.println("AdaptableX509CertSelector.match: subject key IDs don't match"); 
        return false;
      } 
