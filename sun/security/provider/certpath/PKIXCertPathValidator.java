...
public final class PKIXCertPathValidator extends CertPathValidatorSpi {
  ...
  private static PKIXCertPathValidatorResult validate(PKIX.ValidatorParams paramValidatorParams) throws CertPathValidatorException {
    ...
    List<X509Certificate> list = paramValidatorParams.certificates();
    if (!list.isEmpty()) {
      ...
      try {
        ...
        adaptableX509CertSelector.setSkiAndSerialNumber(x509CertImpl
    ...
    for (TrustAnchor trustAnchor : paramValidatorParams.trustAnchors()) {
      ...
      if (x509Certificate != null) {
        ...
        if (debug != null) {
          ...
          debug.println("anchor.getTrustedCert().getSubjectX500Principal() = " + x509Certificate
  ...
  private static PKIXCertPathValidatorResult validate(TrustAnchor paramTrustAnchor, PKIX.ValidatorParams paramValidatorParams) throws CertPathValidatorException {
    ...
    ArrayList<UntrustedChecker> arrayList = new ArrayList();
    arrayList.add(untrustedChecker);
    arrayList.add(new AlgorithmChecker(paramTrustAnchor));
    arrayList.add(new KeyChecker(i, paramValidatorParams
    ...
    BasicChecker basicChecker = new BasicChecker(paramTrustAnchor, paramValidatorParams.date(), paramValidatorParams.sigProvider(), false);
    arrayList.add(basicChecker);
    boolean bool = false;
    List<PKIXCertPathChecker> list = paramValidatorParams.certPathCheckers();
    ...
    PKIXMasterCertPathValidator.validate(paramValidatorParams.certPath(), paramValidatorParams
        .certificates(), (List)arrayList);
    return new PKIXCertPathValidatorResult(paramTrustAnchor, policyChecker.getPolicyTree(), basicChecker
        .getPublicKey());
  }
}
