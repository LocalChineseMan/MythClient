...
public class PrincipalName implements Cloneable {
  ...
  public PrincipalName(String[] paramArrayOfString, int paramInt) throws IllegalArgumentException, RealmException {
    this(paramInt, paramArrayOfString, Realm.getDefault());
  }
  ...
  static {
    try {
      ...
      NAME_STRINGS_OFFSET = unsafe.objectFieldOffset(PrincipalName.class
          .getDeclaredField("nameStrings"));
      UNSAFE = unsafe;
  ...
  public boolean equals(Object paramObject) {
    ...
    if (paramObject instanceof PrincipalName) {
      ...
      return (this.nameRealm.equals(principalName.nameRealm) && 
        Arrays.equals((Object[])this.nameStrings, (Object[])principalName.nameStrings));
    } 
    return false;
  }
  ...
  public PrincipalName(DerValue paramDerValue, Realm paramRealm) throws Asn1Exception, IOException {
    ...
    if ((derValue.getTag() & 0x1F) == 1) {
      ...
      vector.copyInto((Object[])this.nameStrings);
  ...
  private static String[] parseName(String paramString) {
    ...
    while (b < str.length()) {
      if (str.charAt(b) == '/') {
        if (b > 0 && str.charAt(b - 1) == '\\') {
          str = str.substring(0, b - 1) + str.substring(b, str.length());
          continue;
        } 
        if (i <= b) {
          String str1 = str.substring(i, b);
  ...
  public boolean match(PrincipalName paramPrincipalName) {
    boolean bool = true;
    if (this.nameRealm != null && paramPrincipalName.nameRealm != null && 
      !this.nameRealm.toString().equalsIgnoreCase(paramPrincipalName.nameRealm.toString()))
      bool = false; 
