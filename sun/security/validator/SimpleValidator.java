...
public final class SimpleValidator extends Validator {
  ...
  X509Certificate[] engineValidate(X509Certificate[] paramArrayOfX509Certificate, Collection<X509Certificate> paramCollection, AlgorithmConstraints paramAlgorithmConstraints, Object paramObject) throws CertificateException {
    ...
    try {
      ...
      throw new ValidatorException("Untrusted certificate: " + x509Certificate
          .getSubjectX500Principal(), ValidatorException.T_UNTRUSTED_CERT, x509Certificate, certPathValidatorException);
    } 
    TrustAnchor trustAnchor = new TrustAnchor(x509Certificate, null);
    AlgorithmChecker algorithmChecker1 = new AlgorithmChecker(trustAnchor);
    AlgorithmChecker algorithmChecker2 = null;
    if (paramAlgorithmConstraints != null)
      algorithmChecker2 = new AlgorithmChecker(trustAnchor, paramAlgorithmConstraints); 
    ...
    for (int j = paramArrayOfX509Certificate.length - 2; j >= 0; j--) {
      ...
      try {
        ...
        throw new ValidatorException("Untrusted certificate: " + x509Certificate2
            .getSubjectX500Principal(), ValidatorException.T_UNTRUSTED_CERT, x509Certificate2, certPathValidatorException);
      } 
      try {
      ...
      if (!this.variant.equals("code signing") && 
        !this.variant.equals("jce signing"))
        x509Certificate2.checkValidity(date); 
      if (!x509Certificate2.getIssuerX500Principal().equals(x509Certificate1
  ...
  private int checkExtensions(X509Certificate paramX509Certificate, int paramInt) throws CertificateException {
    Set<String> set = paramX509Certificate.getCriticalExtensionOIDs();
    if (set == null)
  ...
  private void checkNetscapeCertType(X509Certificate paramX509Certificate, Set<String> paramSet) throws CertificateException {
    if (!this.variant.equals("generic"))
      if (this.variant.equals("tls client") || this.variant
        .equals("tls server")) {
      ...
      } else if (this.variant.equals("code signing") || this.variant
        .equals("jce signing")) {
  ...
  private void checkKeyUsage(X509Certificate paramX509Certificate, Set<String> paramSet) throws CertificateException {
    ...
    if (arrayOfBoolean != null)
      if (arrayOfBoolean.length < 6 || !arrayOfBoolean[5])
  ...
  private X509Certificate[] buildTrustedChain(X509Certificate[] paramArrayOfX509Certificate) throws CertificateException {
    ...
    for (byte b = 0; b < paramArrayOfX509Certificate.length; b++) {
      ...
      if (x509Certificate2 != null) {
        ...
        return arrayList.<X509Certificate>toArray(CHAIN0);
      } 
    ...
    List<X509Certificate> list = this.trustedX500Principals.get(x500Principal2);
    if (list != null) {
      X509Certificate x509Certificate1 = list.iterator().next();
      arrayList.add(x509Certificate1);
      return arrayList.<X509Certificate>toArray(CHAIN0);
    } 
  ...
  private X509Certificate getTrustedCertificate(X509Certificate paramX509Certificate) {
    ...
    List list = this.trustedX500Principals.get(x500Principal1);
    if (list == null)
      return null; 
    ...
    for (X509Certificate x509Certificate : list) {
      ...
      if (!x509Certificate.getIssuerX500Principal().equals(x500Principal2))
        continue; 
