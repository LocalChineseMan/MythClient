...
public class JRELocaleProviderAdapter extends LocaleProviderAdapter implements ResourceBundleBasedAdapter {
  ...
  public LocaleResources getLocaleResources(Locale paramLocale) {
    LocaleResources localeResources = this.localeResourcesMap.get(paramLocale);
    if (localeResources == null) {
      localeResources = new LocaleResources((ResourceBundleBasedAdapter)this, paramLocale);
      LocaleResources localeResources1 = this.localeResourcesMap.putIfAbsent(paramLocale, localeResources);
  ...
  public Locale[] getAvailableLocales() {
    return (Locale[])AvailableJRELocales.access$000().clone();
  }
  
  public Set<String> getLanguageTagSet(String paramString) {
    Set<String> set = this.langtagSets.get(paramString);
    if (set == null) {
      set = createLanguageTagSet(paramString);
      Set<String> set1 = this.langtagSets.putIfAbsent(paramString, set);
      if (set1 != null)
        set = set1; 
    } 
    return set;
    ...
    if (isNonENSupported == null)
      synchronized (JRELocaleProviderAdapter.class) {
        if (isNonENSupported == null) {
          ...
          String str2 = (String)AccessController.<String>doPrivileged(new GetPropertyAction("java.home")) + str1 + "lib" + str1 + "ext" + str1 + "localedata.jar";
          final File f = new File(str2);
          isNonENSupported = AccessController.<Boolean>doPrivileged(new PrivilegedAction<Boolean>() {
                public Boolean run() {
  ...
  private static class JRELocaleProviderAdapter {}
}
