...
public final class StringPrep {
  ...
  private static final class StringPrep {}
  ...
  private char getCodePointValue(int paramInt) {
    return this.sprepTrieImpl.sprepTrie.getCodePointValue(paramInt);
  }
  ...
  public StringPrep(InputStream paramInputStream) throws IOException {
    ...
    if (versionInfo.compareTo(this.sprepUniVer) < 0 && versionInfo
      .compareTo(this.normCorrVer) < 0 && (this.indexes[7] & 0x1) > 0)
  ...
  private StringBuffer map(UCharacterIterator paramUCharacterIterator, int paramInt) throws ParseException {
    ...
    boolean bool = ((paramInt & 0x1) > 0) ? true : false;
    while ((i = paramUCharacterIterator.nextCodePoint()) != -1) {
      c = getCodePointValue(i);
      getValues(c, values);
      if (values.type == 0 && !bool)
        throw new ParseException("An unassigned code point was found in the input " + paramUCharacterIterator
            .getText(), paramUCharacterIterator.getIndex()); 
      if (values.type == 1) {
        if (values.isIndex) {
          char c1;
          int j = values.value;
  ...
  private StringBuffer normalize(StringBuffer paramStringBuffer) {
    return new StringBuffer(
        Normalizer.normalize(paramStringBuffer
          .toString(), Normalizer.Form.NFKC, 262432));
  }
  ...
  public StringBuffer prepare(UCharacterIterator paramUCharacterIterator, int paramInt) throws ParseException {
    ...
    while ((i = uCharacterIterator.nextCodePoint()) != -1) {
      ...
      getValues(c, values);
      if (values.type == 2)
        throw new ParseException("A prohibited code point was found in the input" + uCharacterIterator
            .getText(), values.value); 
    ...
    if (this.checkBiDi == true) {
      if (bool2 == true && bool1 == true)
        throw new ParseException("The input does not conform to the rules for BiDi code points." + uCharacterIterator
            .getText(), (m > n) ? m : n); 
