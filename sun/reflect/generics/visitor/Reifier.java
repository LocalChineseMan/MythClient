...
public class Reifier implements TypeTreeVisitor<Type> {
  ...
  public void visitFormalTypeParameter(FormalTypeParameter paramFormalTypeParameter) {
    this.resultType = getFactory().makeTypeVariable(paramFormalTypeParameter.getName(), paramFormalTypeParameter
        .getBounds());
  }
  
  public void visitClassTypeSignature(ClassTypeSignature paramClassTypeSignature) {
    List<SimpleClassTypeSignature> list = paramClassTypeSignature.getPath();
    assert !list.isEmpty();
  ...
  public void visitWildcard(Wildcard paramWildcard) {
    this.resultType = getFactory().makeWildcard(paramWildcard.getUpperBounds(), paramWildcard
        .getLowerBounds());
  }
