...
public class SignatureParser {
  ...
  private FormalTypeParameter[] parseFormalTypeParameters() {
    ArrayList<FormalTypeParameter> arrayList = new ArrayList(3);
    assert current() == '<';
    if (current() != '<')
    ...
    advance();
    return arrayList.<FormalTypeParameter>toArray(new FormalTypeParameter[arrayList.size()]);
  }
  ...
  private ClassTypeSignature parseClassTypeSignature() {
    ...
    advance();
    ArrayList<SimpleClassTypeSignature> arrayList = new ArrayList(5);
  ...
  private TypeArgument[] parseTypeArguments() {
    ArrayList<TypeArgument> arrayList = new ArrayList(3);
    assert current() == '<';
    if (current() != '<')
      throw error("expected '<'"); 
    advance();
  ...
  private TypeVariableSignature parseTypeVariableSignature() {
    ...
    if (current() != ';')
      throw error("; expected in signature of type variable named" + typeVariableSignature
          .getIdentifier()); 
    advance();
  ...
  private FieldTypeSignature[] parseBounds() {
    ArrayList<FieldTypeSignature> arrayList = new ArrayList(3);
    if (current() == ':') {
      advance();
      switch (current()) {
        case ':':
    ...
    return arrayList.<FieldTypeSignature>toArray(new FieldTypeSignature[arrayList.size()]);
  }
  
  private ClassTypeSignature[] parseSuperInterfaces() {
    ArrayList<ClassTypeSignature> arrayList = new ArrayList(5);
    while (current() == 'L')
      arrayList.add(parseClassTypeSignature()); 
    return arrayList.<ClassTypeSignature>toArray(new ClassTypeSignature[arrayList.size()]);
  }
  ...
  private TypeSignature[] parseZeroOrMoreTypeSignatures() {
    ArrayList<TypeSignature> arrayList = new ArrayList();
    boolean bool = false;
    while (!bool) {
      switch (current()) {
        case 'B':
    ...
    return arrayList.<TypeSignature>toArray(new TypeSignature[arrayList.size()]);
  }
