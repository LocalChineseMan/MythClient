...
class MethodAccessorGenerator extends AccessorGenerator {
  ...
  public MethodAccessor generateMethod(Class<?> paramClass1, String paramString, Class<?>[] paramArrayOfClass1, Class<?> paramClass2, Class<?>[] paramArrayOfClass2, int paramInt) {
    return (MethodAccessor)generate(paramClass1, paramString, paramArrayOfClass1, paramClass2, paramArrayOfClass2, paramInt, false, false, (Class<?>)null);
  }
  ...
  public ConstructorAccessor generateConstructor(Class<?> paramClass, Class<?>[] paramArrayOfClass1, Class<?>[] paramArrayOfClass2, int paramInt) {
    return (ConstructorAccessor)generate(paramClass, "<init>", paramArrayOfClass1, void.class, paramArrayOfClass2, paramInt, true, false, (Class<?>)null);
  }
  ...
  private MagicAccessorImpl generate(final Class<?> declaringClass, String paramString, Class<?>[] paramArrayOfClass1, Class<?> paramClass2, Class<?>[] paramArrayOfClass2, int paramInt, boolean paramBoolean1, boolean paramBoolean2, Class<?> paramClass3) {
    ...
    if (paramBoolean1) {
      if (paramBoolean2) {
        this.asm
          .emitConstantPoolUTF8("sun/reflect/SerializationConstructorAccessorImpl");
      } else {
    ...
    if (paramBoolean1) {
      ...
      this.asm
        .emitConstantPoolUTF8("(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;");
    } 
    ...
    return AccessController.<MagicAccessorImpl>doPrivileged(new PrivilegedAction<MagicAccessorImpl>() {
          public MagicAccessorImpl run() {
            try {
              return (MagicAccessorImpl)ClassDefiner.defineClass(generatedName, bytes, 0, bytes.length, declaringClass
  ...
  private void emitInvoke() {
      ...
      if (isPrimitive(clazz)) {
        ...
        for (byte b1 = 0; b1 < primitiveTypes.length; b1++) {
          ...
          if (canWidenTo(clazz1, clazz)) {
            ...
            classFileAssembler1.opc_invokevirtual(unboxingMethodForPrimitiveType(clazz1), 0, 
                
                typeSizeInStackSlots(clazz1));
    ...
    if (this.isConstructor) {
      ...
      classFileAssembler1.opc_invokestatic(this.targetMethodRef, b, 
          
          typeSizeInStackSlots(this.returnType));
    } else if (isInterface()) {
      if (isPrivate()) {
        classFileAssembler1.opc_invokespecial(this.targetMethodRef, b, 0);
      } else {
        classFileAssembler1.opc_invokeinterface(this.targetMethodRef, b, b, 
            
            typeSizeInStackSlots(this.returnType));
      } 
    } else {
      classFileAssembler1.opc_invokevirtual(this.targetMethodRef, b, 
    ...
    if (!this.isConstructor)
      if (isPrimitive(this.returnType)) {
        classFileAssembler1.opc_invokespecial(ctorIndexForPrimitiveType(this.returnType), 
            typeSizeInStackSlots(this.returnType), 0);
