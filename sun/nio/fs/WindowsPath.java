...
class WindowsPath extends AbstractPath {
  ...
  static WindowsPath createFromNormalizedPath(WindowsFileSystem paramWindowsFileSystem, String paramString, BasicFileAttributes paramBasicFileAttributes) {
    try {
      ...
      if (paramBasicFileAttributes == null)
        return new WindowsPath(paramWindowsFileSystem, result
            .type(), result
            .root(), result
            .path()); 
      return new WindowsPathWithAttributes(paramWindowsFileSystem, result
          .type(), result
          .root(), result
          .path(), paramBasicFileAttributes);
  ...
  private static class WindowsPathWithAttributes extends WindowsPath implements BasicFileAttributesHolder {
    ...
    WindowsPathWithAttributes(WindowsFileSystem param1WindowsFileSystem, WindowsPathType param1WindowsPathType, String param1String1, String param1String2, BasicFileAttributes param1BasicFileAttributes) {
      super(param1WindowsFileSystem, param1WindowsPathType, param1String1, param1String2);
  ...
  private String getAbsolutePath() throws WindowsException {
    ...
    try {
      ...
      throw new WindowsException("Unable to get working directory of drive '" + 
          Character.toUpperCase(this.root.charAt(0)) + "'");
    } 
    String str2 = str1;
    if (str1.endsWith("\\")) {
  ...
  private static boolean isSameDrive(String paramString1, String paramString2) {
    return 
      (Character.toUpperCase(paramString1.charAt(0)) == Character.toUpperCase(paramString2.charAt(0)));
  }
  ...
  public WindowsPath getParent() {
    ...
    if (i < this.root.length())
      return getRoot(); 
    return new WindowsPath(getFileSystem(), this.type, this.root, this.path
  ...
  public WindowsPath relativize(Path paramPath) {
    ...
    byte b1 = 0;
    while (b1 < k && 
      getName(b1).equals(windowsPath.getName(b1)))
      b1++; 
  ...
  private void initOffsets() {
    if (this.offsets == null) {
      ...
      synchronized (this) {
        if (this.offsets == null)
          this.offsets = arrayList.<Integer>toArray(new Integer[arrayList.size()]); 
      } 
    } 
  }
  ...
  public boolean startsWith(Path paramPath) {
    if (!(Objects.requireNonNull((T)paramPath) instanceof WindowsPath))
      return false; 
  ...
  public WatchKey register(WatchService paramWatchService, WatchEvent.Kind<?>[] paramArrayOfKind, WatchEvent.Modifier... paramVarArgs) throws IOException {
    ...
    if (securityManager != null) {
      ...
      if (i > 0) {
        paramVarArgs = Arrays.<WatchEvent.Modifier>copyOf(paramVarArgs, i);
        byte b = 0;
        while (b < i) {
          if (paramVarArgs[b++] == ExtendedWatchEventModifier.FILE_TREE) {
            bool = true;
            break;
          } 
        } 
