...
public class ProxyGenerator {
  ...
  private static final boolean saveGeneratedFiles = ((Boolean)AccessController.<Boolean>doPrivileged(new GetBooleanAction("sun.misc.ProxyGenerator.saveGeneratedFiles")))
  ...
  private class ProxyMethod {
    ...
    private ProxyGenerator.MethodInfo generateMethod() throws IOException {
      ...
      ProxyGenerator.MethodInfo methodInfo = new ProxyGenerator.MethodInfo(this.methodName, str, 17);
      ...
      dataOutputStream.writeShort(ProxyGenerator.this.cp.getFieldRef(ProxyGenerator
            .dotToSlash(ProxyGenerator.this.className), this.methodFieldName, "Ljava/lang/reflect/Method;"));
      ...
      short s1 = (short)methodInfo.code.size(), s2 = s1;
      List list = ProxyGenerator.computeUniqueCatchList(this.exceptionTypes);
      if (list.size() > 0) {
        for (Class clazz : list)
          methodInfo.exceptionTable.add(new ProxyGenerator.ExceptionTableEntry(bool, s2, s1, ProxyGenerator.this
                
                .cp.getClass(ProxyGenerator.dotToSlash(clazz.getName())))); 
      ...
      for (byte b = 0; b < this.exceptionTypes.length; b++)
        methodInfo.declaredExceptions[b] = ProxyGenerator.this.cp.getClass(ProxyGenerator
            .dotToSlash(this.exceptionTypes[b].getName())); 
      return methodInfo;
    }
    ...
    private void codeFieldInitialization(DataOutputStream param1DataOutputStream) throws IOException {
      ...
      param1DataOutputStream.writeShort(ProxyGenerator.this.cp.getFieldRef(ProxyGenerator
            .dotToSlash(ProxyGenerator.this.className), this.methodFieldName, "Ljava/lang/reflect/Method;"));
    }
  }
  ...
  private MethodInfo generateStaticInitializer() throws IOException {
    ...
    dataOutputStream.writeByte(177);
    short s1 = (short)methodInfo.code.size(), s2 = s1;
    methodInfo.exceptionTable.add(new ExceptionTableEntry(bool, s2, s1, this.cp
  ...
  private static String getMethodDescriptor(Class<?>[] paramArrayOfClass, Class<?> paramClass) {
    return getParameterDescriptors(paramArrayOfClass) + ((paramClass == void.class) ? "V" : 
      getFieldType(paramClass));
  }
  ...
  private static List<Class<?>> computeUniqueCatchList(Class<?>[] paramArrayOfClass) {
    ArrayList<Class<Error>> arrayList = new ArrayList();
  ...
  private static class PrimitiveTypeInfo {
    ...
    private static void add(Class<?> param1Class1, Class<?> param1Class2) {
      table.put(param1Class1, new PrimitiveTypeInfo(param1Class1, param1Class2));
    }
    ...
    private PrimitiveTypeInfo(Class<?> param1Class1, Class<?> param1Class2) {
      ...
      this
        ...
        .baseTypeString = Array.newInstance(param1Class1, 0).getClass().getName().substring(1);
  ...
  private static class ConstantPool {
    ...
    public short getInteger(int param1Int) {
      return getValue(new Integer(param1Int));
    }
    
    public short getFloat(float param1Float) {
      return getValue(new Float(param1Float));
    }
    ...
    private short getIndirect(IndirectEntry param1IndirectEntry) {
      Short short_ = this.map.get(param1IndirectEntry);
      if (short_ != null)
        return short_.shortValue(); 
      if (this.readOnly)
