...
class DiagnosticCommandImpl extends NotificationEmitterSupport implements DiagnosticCommandMBean {
        ...
        try {
          ...
          if (this.permission == null)
            try {
              ...
              this.permission = (Permission)constructor.newInstance(new Object[] { param1DiagnosticCommandInfo
                    .getPermissionName(), param1DiagnosticCommandInfo
                    .getPermissionAction() });
            } catch (InstantiationException|IllegalAccessException|IllegalArgumentException|java.lang.reflect.InvocationTargetException|NoSuchMethodException|SecurityException instantiationException1) {
              instantiationException = instantiationException1;
            }  
  ...
  private static class OperationInfoComparator implements Comparator<MBeanOperationInfo> {
    ...
    public int compare(MBeanOperationInfo param1MBeanOperationInfo1, MBeanOperationInfo param1MBeanOperationInfo2) {
      return param1MBeanOperationInfo1.getName().compareTo(param1MBeanOperationInfo2.getName());
    }
  }
  
  public MBeanInfo getMBeanInfo() {
    Map<? extends String, ? extends Wrapper> map;
    TreeSet<MBeanOperationInfo> treeSet = new TreeSet(new OperationInfoComparator());
    if (!this.isSupported) {
      ...
      try {
        ...
        for (byte b = 0; b < arrayOfString.length; b++) {
          ...
          try {
            ...
            map.put(str, wrapper);
            treeSet.add(new MBeanOperationInfo(wrapper.name, wrapper.info
                  
                  .getDescription(), (wrapper.info
                  .getArgumentsInfo() == null || wrapper.info
                  .getArgumentsInfo().isEmpty()) ? null : arrayOfMBeanParameterInfo, strClassName, 2, 
    ...
    ImmutableDescriptor immutableDescriptor = new ImmutableDescriptor((Map)hashMap);
    return new MBeanInfo(
        getClass().getName(), "Diagnostic Commands", null, null, (MBeanOperationInfo[])treeSet
        
        .toArray((Object[])new MBeanOperationInfo[treeSet.size()]), 
        getNotificationInfo(), immutableDescriptor);
  }
  ...
  public Object invoke(String paramString, Object[] paramArrayOfObject, String[] paramArrayOfString) throws MBeanException, ReflectionException {
    ...
    if (this.wrappers == null)
      getMBeanInfo(); 
    Wrapper wrapper = this.wrappers.get(paramString);
    if (wrapper != null) {
      ...
      if (paramArrayOfObject != null && paramArrayOfObject.length == 1 && paramArrayOfString != null && paramArrayOfString.length == 1 && paramArrayOfString[0] != null && paramArrayOfString[0]
        
        .compareTo(strArrayClassName) == 0)
  ...
  private Descriptor commandDescriptor(Wrapper paramWrapper) throws IllegalArgumentException {
    ...
    if (paramWrapper.info.getArgumentsInfo() != null && !paramWrapper.info.getArgumentsInfo().isEmpty()) {
      ...
      for (DiagnosticCommandArgumentInfo diagnosticCommandArgumentInfo : paramWrapper.info.getArgumentsInfo()) {
        ...
        hashMap1.put(diagnosticCommandArgumentInfo.getName(), new ImmutableDescriptor((Map)hashMap2));
      } 
      hashMap.put("dcmd.arguments", new ImmutableDescriptor((Map)hashMap1));
    } 
